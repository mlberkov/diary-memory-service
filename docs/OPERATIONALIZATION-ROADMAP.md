...
reconciliation entrypoint; the OP-3.2 retry slice that resolves A-35 is not yet
...
| **OP-3** *(in progress — OP-3.1 / D-050 landed; OP-3.2 open)* | Failed-embedding reconciliation | OP-1 and OP-2 merged — OP-3 consumes OP-2's bounded-backoff retry (6.1) and dead-letter (6.2) primitives, per A-35's own specification. | A-35 resolved (`docs/assumptions.md`), governed by the same Phase 6 Definition of Done. OP-3.1 landed the read-only discovery seam but does **not** resolve A-35; OP-3.2 (retry) does. | `make check`; a `failed` chunk is retried, succeeds or lands in the dead-letter surface, and the outcome is observable. |
