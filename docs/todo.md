...
## Phase 4 — Slice 4.3b fallback grading (next, after Slice 4.3a)
- Owner: agent
- Map: execution-map 4.3b → 4.4
- Concrete: with `ChatClient` + `AnswerTrace` persistence landed for the success and no-evidence contours in Slice 4.3a (D-034), add the fallback-grading contour the bounded packet deferred. Decide the marker shape — whether `UncertaintyMarker` extends beyond `{confident, uncertain, no_evidence}`, whether TechSpec §5's `confidence_band` becomes a real column or stays out, and how parse-failure / provider-unavailable / weak-evidence / ambiguous outcomes are graded. Wire `QueryService.answer` to persist an `AnswerTrace` row on each of those contours and surface them through the dispatcher / Telegram reply layer per R-6 (requested vs effective path). Stay bounded — do not bundle real-provider integration (Phase 6) or Telegram citation rendering (Slice 4.4).
