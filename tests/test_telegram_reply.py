"""Reply-formatter and dispatcher reply-text tests."""
...
def test_dispatcher_start_reply_mentions_diary_mode() -> None:
    result = Dispatcher().dispatch(_inbound(RouteKind.START, "/start"))
...
def test_dispatcher_help_reply_lists_supported_commands() -> None:
    result = Dispatcher().dispatch(_inbound(RouteKind.HELP, "/help"))
    text = result.reply_text
    for token in ("/start", "/help", "/entry", "/ask"):
        assert token in text


def test_dispatcher_entry_reply_acknowledges_mock() -> None:
    result = Dispatcher().dispatch(_inbound(RouteKind.ENTRY, "/entry foo", "foo"))
    assert "mock" in result.reply_text.lower()
    assert result.metadata.get("mock") == "true"


def test_dispatcher_ask_reply_acknowledges_mock() -> None:
    result = Dispatcher().dispatch(_inbound(RouteKind.ASK, "/ask foo", "foo"))
    assert "mock" in result.reply_text.lower()
    assert result.metadata.get("mock") == "true"


def test_dispatcher_unknown_reply_points_at_help() -> None:
    result = Dispatcher().dispatch(_inbound(RouteKind.UNKNOWN, "hello"))
    text = result.reply_text
