...
class Settings(BaseSettings):
    ...
    postgres_db: str = Field(default="memory_rag")
    postgres_user: str = Field(default="postgres")
    postgres_password: str = Field(default="postgres")

    storage_backend: Literal["memory", "sqlite", "postgres"] = Field(default="memory")
    sqlite_path: str = Field(default="./data/memory_rag.db")

    embedding_backend: Literal["mock", "openai"] = Field(default="mock")
    embedding_model: str = Field(default="text-embedding-3-large")
    embedding_dimension: int = Field(default=3072)
    chat_backend: Literal["mock", "openai"] = Field(default="mock")
    # Load-bearing for the openai backend (D-037): boot aborts when
