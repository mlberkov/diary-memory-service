...
### Local Postgres
The canonical durable backend (D-022) runs via `docker compose up -d postgres`. Set `STORAGE_BACKEND=postgres` and the standard `POSTGRES_*` env vars. The compose image is `pgvector/pgvector:pg16` (D-024) so the `vector` extension is available for the embedding seam. The host publish is loopback-only (`127.0.0.1:${POSTGRES_PORT:-5432}:5432`): app containers and sidecars use the Compose network (`postgres:5432`), and the DB port must not be reachable from the public Internet. See `QUICKSTART.md` "Durable local store (Postgres)" for the full smoke flow.
...
`/sources` exposes the chunks the LLM **cited** in the chat's most recent `/ask` answer (`AnswerResult.cited_chunk_ids` — an I-9 subset of the retrieved context; D-098 / D-100), **not** the full retrieved set. This evolved from the original behavior: earlier (D-036) `/sources` returned every post-RRF chunk `services/context_assembler.assemble_answer_context` produced and `build_answer_prompt` fed to the LLM; since D-100 it returns only the cited subset, kept in post-RRF order. Each cited chunk is rendered with `note_date`, a 1-based `(i/N)` index, a `— <author>` attribution line, and the full `chunk_text` verbatim. Block layout is `[YYYY-MM-DD] (i/N)` then a `— <author>` line then the verbatim text (D-086). The raw `chunk_id` is **not** surfaced to the user (D-069); operator forensics still join through `AnswerTrace.context_chunk_ids` (the full retrieved context — a superset of the cited subset) via the SQL recipe in "Inspecting recent `/ask` retrieval traces (D-032)" above. The `(i/N)` marker is **per-last-`/ask` ephemeral ordering, not a stable cross-`/ask` identifier** — the index numbers the chunks within the current cached list in post-RRF order, and the cache is overwritten by the next `/ask`. It is not citations, not fine-grained attribution, and not the full pre-RRF candidate pool. The dispatcher returns the opaque chunks (`DispatchResult.source_chunks`) and the Telegram adapter renders each block and resolves the author display name, so display resolution stays adapter-only (D-081 / D-086) — mirroring how `/drafts` blocks are rendered adapter-side. Outbound delivery is one Telegram message by default and splits across multiple messages only when the 4096-char cap forces it (whole-block boundaries; `(part k/N)` footers on an oversized single chunk; identical packing semantics to `/drafts`). The `(i/N)` block header and the outbound `(part k/N)` packing footer live at distinct positions in the rendered message and do not collide.

The state behind `/sources` is a process-local `Dispatcher._latest_sources: dict[str, tuple[EventChunk, ...]]` keyed by `community_id`. The current FastAPI wiring at `adapters/telegram/webhook.py` makes `Dispatcher` a module-level singleton via `get_dispatcher()`, so `/ask` and a follow-up `/sources` are served by the same instance within one process. **Every `/ask` dispatch writes the cache** (no skip path), so key presence records that an `/ask` happened at all. The stored value is the *cited* subset of `answer.context.ordered_chunks` — the chunks in `answer.cited_chunk_ids` (D-098), in post-RRF order — which is empty for every cited-empty contour (both `NO_EVIDENCE` paths, `PROVIDER_UNAVAILABLE`, `PARSE_FAILURE`; D-099). The next `/ask` overwrites the entry; the key is never removed, so `/sources` distinguishes "no prior `/ask`" (key absent) from "asked, but cited nothing" (key present, empty tuple). Non-`/ask` routes never touch the cache. `/sources` itself is read-only.

Fail-closed: when no `/ask` has run in this process (key absent — also after a process restart), `/sources` returns `"No selected chunks available — ask a question with /ask first."`. When the most recent `/ask` cited nothing (key present, empty — every cited-empty contour above), it returns `"Your last /ask answer didn't cite any specific notes."`. Both replies use the inline `sendMessage` body — no outbound HTTP call.
...
Author attribution (D-081 / D-086): `/sources` renders the author display name on a `— <author>` line per block. It is resolved **only at the Telegram adapter seam** (`adapters/telegram/author_display.py`) from the host-supplied display-input snapshot captured under D-084, with the fallback chain `username → first_name → opaque short-ID`: a non-blank `username` renders as `@<username>`, else a non-blank `first_name` renders plain, else the opaque floor `user-<last 8 of author_user_id>`. The floor also covers a both-null/withheld snapshot or a chunk with no captured snapshot (e.g. pre-D-084), so the line is never blank and never raises. The resolution bridges the opaque boundary by looking the `source_message_id` up (`get_source_message`) to recover the `(external_chat_id, external_message_id, edit_seq)` tuple that keys `author_display_inputs`, then reading the snapshot — `EventChunk` itself carries only the opaque `author_user_id`. Resolved names are **non-authoritative** presentation (a Telegram user may change or withhold them); the core still carries authorship only as the opaque `author_user_id`. `/sources` is the sole sanctioned display surface this milestone; answer-reply (`/ask` reply) attribution is deferred (see `docs/assumptions.md` A-44).

Answer-reply contributor attribution — removed (D-101): the `/ask` reply once carried a `Contributors: …` footer (contract D-091, code D-092). It was removed as a user-facing element because it rendered the **full retrieved set** while `/sources` (cited-only, D-100) renders only the LLM-cited chunks — two attribution surfaces on the same answer that disagreed. `/sources` is now the **sole** user-facing attribution surface; the `/ask` reply is the answer text alone. No invariant changed (authorship is still carried only as the opaque `author_user_id`; `/sources` still resolves names adapter-side, requester-`community_id`-scoped per D-089 — I-1 / I-6 / I-7 unchanged). `AnswerTrace.context_chunk_ids` still records the full retrieved context for operator forensics.

### Read-access scoping (Slice 8.1 / D-087, D-088, D-089, D-090)
Slice 8.1 hardens community scoping on every read so cross-community leakage is prevented and access behavior is explicit (Phase-8 DoD; I-7 / R-3 / R-8). What it enforces, for the operator: (How a `community_id` is *assigned* — implicit-on-first-message bootstrap; default Telegram 1:1 from `external_chat_id`; membership inherited from host-chat membership; many communities per instance — is ratified in D-093 / `docs/GROUPED-MULTI-DIARY-ROADMAP.md`; this section governs reads, not assignment.)

- **Every read of a community-owned record is community-scoped.** A read either carries a non-null `community_id` and filters by the owning community, or is a documented safe-by-construction seam. The five scoped by-id / trace / source reads are `get_query`, `get_retrieval_hits_for_query`, `get_answer_trace_for_query`, `get_event_chunk`, and `get_source_message`. The hot `/ask` retrieval path (`dense_candidates` / `sparse_candidates`) and the list reads (`list_source_messages`, `list_recent_drafts`, `list_failed_event_chunks`) were already scoped before 8.1.
- **Fail-closed on a bad community id.** A missing/empty `community_id` raises `ValueError("community_id is required (Runtime invariant R-3)")` — no log-and-continue, no admin bypass. A read for a *different* community returns the empty sentinel (`None`, or `[]` for `get_retrieval_hits_for_query`), never another community's row. On the live `/sources` author-resolution path a cross-community `get_source_message` reads `None` and falls through to the opaque author floor (`user-<last8>`) rather than raising — the rendered block is byte-unchanged.
- **Scoping shape — own-column filter vs. join.** Records that carry `community_id` directly (`source_messages`, `event_chunks`, `queries`) filter on their own column (`WHERE … AND community_id = …`; the mock compares the stored row). Trace records whose community lives on the parent (`get_answer_trace_for_query`, `get_retrieval_hits_for_query`) scope via the `query_id → queries.community_id` join. A parent-missing trace read is fail-closed (`None` / `[]`).
- **Keyword-only `community_id`.** On the scoped by-id reads `community_id` is keyword-only, preventing a silent positional swap between two `str` identifiers.
- **No `answer_traces` schema change.** `answer_traces` has no `community_id` column; community is recovered through the `queries` join. Slice 8.1 is read-only — no column, DDL, or migration.
- **`/sources` cache is safe by construction.** `Dispatcher._latest_sources` is keyed by `community_id` (D-036), so it never serves another community's chunks; 8.1 threads the requester-scoped `community_id` through author resolution as defense in depth on top of that (D-089).

Verification: the consolidated cross-community isolation sweep `tests/test_read_access_isolation.py` pins the guard + fail-closed contract for all five scoped reads across mock / sqlite / postgres from one parametrized fixture; per-read isolation also lives in `test_storage_query_traces.py`, `test_storage_answer_traces.py`, and `test_storage_source_messages.py`, with the hot-path / prompt-assembly / `/sources`-seam / cache evidence in `test_search_repository_*`, `test_query_service.py`, `test_answer_prompt.py`, `test_author_display_resolution.py`, and `test_dispatcher_sources.py`. Operator caveat: the Postgres leg of these sweeps runs only when `MEMORY_RAG_PG_TEST_DSN` is set; otherwise verification is mock + sqlite.

### Grouped & multi-diary on one instance (D-093, D-094, D-095, D-096)
Grouped diaries (a shared chat) and multiple diaries on one instance are **supported** today; this section is the operator how-to for how a chat becomes a community, who can read it, and how many can coexist. The companion "Read-access scoping (Slice 8.1 …)" section above governs *reads*; this one governs *assignment* (how a `community_id` comes to be). The contract is ratified in D-093 and `docs/GROUPED-MULTI-DIARY-ROADMAP.md`; the mapping was consolidated into one seam by D-094 (G-1) and pinned by `tests/test_grouped_multi_diary.py` (D-095 / G-2).

- **Bootstrap is implicit-on-first-message.** A community is created the first time a chat sends a message — the first inbound message persists a `SourceMessage` and its scope is resolved then. There is **no `/setup` command** and no onboarding step; nothing to provision per chat. (An explicit `/setup` is deferred-optional, not built.)
- **Chat → community mapping (default 1:1).** The Telegram chat is mapped to a core community scope at the webhook edge by the adapter-owned resolver `resolve_community_id(external_chat_id) -> str` (`adapters/telegram/community.py`); the resolved **opaque** `community_id` crosses into the core on `InboundMessage.community_id`. The default mapping is one community per chat (identity), so under the default a chat's `community_id` equals its `external_chat_id` value — but the core never re-derives scope from the chat id (I-1). `external_chat_id` is retained only as the transport / R-2 idempotency key (`(external_chat_id, external_message_id, edit_seq)`) and for addressing replies. A future host that needs a different chat→community policy changes only this one resolver, with no core call-site edits.
- **Grouped diary = one shared community, preserved authorship.** A Telegram **group** chat is one shared `community_id`; every sender writes into and may query the same community corpus, while authorship stays preserved per opaque `author_user_id` (I-6) — so `/sources` attributes lines to the right sender (D-086; the `/ask` `Contributors:` footer was removed in D-101). Membership is **inherited from host-chat membership**: anyone Telegram admits to the chat is a participant for community-level read/query access. There is **no core participant / ACL / membership table** — community-level scoping is the access model.
- **Multiple diaries on one instance.** Distinct chats map to distinct communities, and every durable record is `community_id`-keyed with every read community-scoped and fail-closed (I-7 / R-3 / R-8; Slice 8.1). So one running instance serves N isolated communities (private 1:1 chats and shared group chats side by side) with no cross-community leakage; no per-community process, database, or configuration is required. DEPLOY-1's single-community pilot default (A-42) is a deployment posture, not a core limit.
- **Access granularity / what is not here.** Access is **community-level**: every chat member can query the whole community corpus. **Per-note or per-participant visibility** (e.g. a member keeping notes others in the chat cannot read) is **not** available — it is the visibility model (A-15 / Slice 8.2 / G-4), deferred and sequenced after the first grouped pilot. Subject/child scoping within a community is a separate, finer dimension: its contract is ratified (A-45 **closed → D-097**) and the code has **landed** (Milestone H / H-1..H-4 in `docs/SUBJECT-SCOPING-ROADMAP.md`) — see "Subject scoping within a community" below. It is orthogonal to visibility (subject = *what a note is about*; visibility = *who may see it*).

### Subject scoping within a community (D-097, D-107)
Subject scoping is the second, finer scoping dimension on the community-scoped data plane: an **opaque, community-scoped, nullable** `subject_id` records *what a note is about* (the first use case labels it "child", but the core field is `subject_id` — D-041). The contract is ratified in D-097 and `docs/SUBJECT-SCOPING-ROADMAP.md`; the strict-match retrieval filter is D-107; the end-to-end behavior is pinned by `tests/test_subject_scoping.py` (H-4).

- **Assignment is adapter-owned, default community-wide.** Host→subject assignment happens at the webhook edge via the adapter-owned resolver `resolve_subject_id(community_id) -> str | None` (`adapters/telegram/subject.py`), parallel to `resolve_community_id`; the resolved opaque value crosses into the core on `InboundMessage.subject_id` and is persisted onto `Note` / `EventChunk`. The default single-subject mapping returns `None` = **community-wide**, so under the default every record is community-wide today — subject scoping is additive and does not retro-scope existing records. A host that needs a different policy changes only this one resolver.
- **Retrieval filter is optional and strict-match.** `QueryService.answer` takes a keyword-only `subject_scope: str | None = None`, mirrored on both `SearchRepository` legs (D-040 `date_range` precedent, composing with it as a conjunction). `None` (the default) applies no constraint; a non-`None` scope returns **only** rows with that exact `subject_id` — community-wide (`null`) rows are excluded and reachable only via the default. A scope with no matching rows fails closed to `NO_EVIDENCE`. The filter never widens community scope (I-7 / R-3 stay the outer boundary; the same `subject_id` string in two communities stays isolated).
- **No user-facing surface yet.** There is **no** inbound `/ask` subject syntax, no subject-selection command, and no core subject registry/entity — the filter is reachable through the `QueryService.answer` API, and the dispatcher passes the default. For forensics, the requested scope is recorded on the persisted `Query` row (`queries.subject_scope`, nullable; `0006.query-subject-scope` migration), so an operator can see which scope a query ran under.
- **Separate from visibility.** Subject scoping does not control who may read anything; per-note / per-participant visibility stays the A-15 / Slice 8.2 / G-4 model, deferred.

Verification: `tests/test_subject_scoping.py` pins the cross-seam contract (resolver → `InboundMessage.subject_id` → persisted rows → scoped `answer()` → persisted `Query.subject_scope`) parametrized over mock + Postgres; leg-level predicates live in `tests/test_search_repository_{mock,postgres}.py`, service cases in `tests/test_query_service.py`, round-trips in `tests/test_storage_query_traces.py`. Operator caveat: the Postgres legs run only when `MEMORY_RAG_PG_TEST_DSN` is set (point it at a dedicated test database — the suites truncate it); otherwise verification is mock-only.

### Routed chat (D-108, D-109, D-110, D-111)
`/chat <question>` is the routed conversational entry point: a `gpt-4.1-mini` function-calling classifier names one of four routes — `diary_lookup` / `diary_plus_llm` / `diary_plus_web` / `general_llm` (product register; core identifiers `notes_lookup` / `notes_plus_model` / `notes_plus_knowledge` / `model_only`, `docs/GLOSSARY.md`) — and the service answers under per-segment provenance labels (generalized I-9): notes claims cite chunk ids through the cited-only surface, web claims cite the search result URLs verbatim, model knowledge renders under its explicit label. The contract is D-108; the packets are RC-2..RC-4 (D-109 / D-110 / D-111) and `docs/ROUTED-CHAT-ROADMAP.md` carries the sequence.

- **Knobs.** `CLASSIFIER_BACKEND` (`mock`|`openai`) + `CLASSIFIER_MODEL` (canonical `gpt-4.1-mini`, boot-gated — D-109; the query rewriter and the outward rewriter ride this same contour with no knobs of their own); `KNOWLEDGE_BACKEND` (`mock`|`tavily`) + `TAVILY_API_KEY` (required for `tavily`, boot-gated — D-111) + `KNOWLEDGE_MAX_RESULTS` (default 5 — A-47); the shared `PROVIDER_*` resilience knobs (R-9) govern every provider call on the surface. With `KNOWLEDGE_BACKEND=mock` (the default) the web route still dispatches but the mock source returns no excerpts, so replies carry no web segment.
- **Boot gates (R-10).** `_verify_classifier_contour` (canonical pin; classifier + both rewriters build through the shared factories) and `_verify_knowledge_contour` (`tavily` requires a key; the source builds; `provider_name` non-empty) abort start on mismatch; `app.created` logs `classifier_backend= classifier_model= knowledge_backend=`.
- **Fallback policy (D-108 — no numeric thresholds).** Classification failure, unusable classifier output, an empty question, and a `diary_plus_web` request with no knowledge source wired all funnel cause-neutrally to `diary_lookup`; the reply carries one neutral reroute trailer and the requested-vs-effective distinction lives in the decision row and metadata (R-6), never in cause-specific prose. Inside the mixed routes, degradation is in-route: a failed retrieval-side rewrite searches the notes with the original question; a failed outward rewrite searches the web with the stripped original question; a failed web search proceeds with an empty web plane (generation still runs — empty/weak diary evidence is stated strictly before any web/model content). Logs name causes; user-facing wording does not.
- **Trace plane (read with plain SQL; all rows community-scoped, fail-closed reads).** One `chat_route_decisions` row per `/chat` call (0007 — requested vs effective route, classifier provenance, nullable `query_id` link); one `chat_query_rewrites` row per mixed-route execution (0008 — nullable rewritten query + date bounds; `NULL` = degraded to the original question); one `chat_knowledge_searches` row per `diary_plus_web` execution (0009 — the outward query actually searched, outward-rewriter provenance, `provider_name`, `result_count`, the provider's verbatim response body; `raw_output=''` + `result_count=0` = the search failed and the route degraded). The routes' `Query` / `RetrievalHit` / `AnswerTrace` rows land in the existing plane; mixed-route traces store the raw segmented JSON verbatim (`prompt_version` `notes-plus-model-v1` / `notes-plus-knowledge-v1`).
- **Log families.** `chat.routed` (every call; decision id, requested/effective, fallback), `chat.enriched` / `chat.knowledge` (per mixed-route execution; leg/excerpt counts, rewrite flags), `chat.rewrite_failed` / `chat.outward_rewrite_failed` / `chat.knowledge_failed` (cause-named degradations), and the shared `provider.attempt` / `provider.exhausted` lines (labels `openai.route_classifier`, `openai.query_rewriter`, `openai.outward_rewriter`, `tavily.search`).
- **Privacy note (D-111).** On `diary_plus_web`, the outward query sent to the external search provider is deliberately conditioned on retrieved note content — the system prompt instructs the rewriter to fold in only sharpening specifics, never note text wholesale and never names of people, but operators should know distilled personal context leaves the instance on this route. The exact outward query is auditable per call in `chat_knowledge_searches.outward_query`.
- **Escalation prompt invariant (D-108).** Both mixed routes carry the byte-pinned escalation clause (general developmental information allowed; never a diagnosis or symptom interpretation; potential red flags ⇒ recommend consulting a qualified specialist) in every generation call — one shared constant, test-asserted on red-flag-style prompts with mock providers.
- **`/sources` stays `/ask`-only (D-111).** Routed answers do not populate the `/sources` cache; the decision is recorded in D-111 and test-pinned in both directions.

Verification: `tests/test_routed_chat_milestone.py` pins all four routes end-to-end at service granularity parametrized over mock + Postgres; per-packet suites are indexed in its docstring (`test_routed_chat_service.py`, `test_routed_chat_notes_plus_model.py`, `test_routed_chat_notes_plus_knowledge.py`, `test_dispatcher_chat.py`, the three storage-trace suites, `test_postgres_migrations.py`, `test_end_to_end_smoke.py`). Operator caveat: the Postgres legs run only when `MEMORY_RAG_PG_TEST_DSN` is set; otherwise verification is mock-only. No live OpenAI or Tavily call runs under `make check`.

#### Routed-chat real-backend smoke (RC closure drill / D-111)
The milestone's exit criterion (roadmap §6) requires one operator-deliberate, off-CI round trip proving all four routes against the real backend — mirroring REAL-1 (D-073 / D-074): `make check` runs no live provider call; this procedure is invoked out-of-band and closes the milestone when its dated artifact is committed.

Pre-conditions: `STORAGE_BACKEND=postgres` with OP-1 migrations applied (0001..0009); `EMBEDDING_BACKEND=openai` / `EMBEDDING_MODEL=text-embedding-3-large` / `EMBEDDING_DIMENSION=3072`; `CHAT_BACKEND=openai` / `CHAT_MODEL=gpt-4.1`; `CLASSIFIER_BACKEND=openai` / `CLASSIFIER_MODEL=gpt-4.1-mini`; `KNOWLEDGE_BACKEND=tavily` with a real `TAVILY_API_KEY`; a real `OPENAI_API_KEY`; the webhook secret set. The D-047 / D-049 resilience defaults are active (a transient retry surfacing as `attempt=2/3 outcome=success` is captured verbatim, not treated as a defect).

Procedure:
1. Boot and confirm the `app.created` line carries the canonical backends (both gates passed).
2. Ingest one `/note` with content the routed questions can ground on; confirm the chunk reaches `embedding_status=ready`.
3. Drive one round trip per route through `POST /telegram/webhook`, steering with natural questions (a notes question for `diary_lookup`; a notes+advice question for `diary_plus_llm`; a notes+world question for `diary_plus_web`; a general question for `general_llm`). The classifier is live — record the requested vs effective route from `chat_route_decisions` rather than pre-asserting it; a reroute is captured, not a defect.
4. For each call capture verbatim: the user-facing reply text; the `chat_route_decisions` row; the `chat_query_rewrites` / `chat_knowledge_searches` rows where the route writes them (for `diary_plus_web`, the `outward_query` actually searched and the cited URLs rendered in the reply); the `answer_traces` row (`prompt_version`, `fallback_mode`, `latency_ms`, token counts); and the relevant `chat.*` / `provider.attempt` log lines.
5. Copy `docs/routed-chat-drill/routed-chat-smoke-TEMPLATE.json` to `docs/routed-chat-drill/routed-chat-smoke-<YYYYMMDD>-evidence.json` (UTC-dated), drop the top-level `"_template": true` flag, and replace every `<TO_FILL_BY_OPERATOR>` placeholder with the verbatim captured observation.
6. Redaction rule (REAL-1 precedent): credential-bearing values (`OPENAI_API_KEY`, `TAVILY_API_KEY`, `TELEGRAM_BOT_TOKEN`, `TELEGRAM_WEBHOOK_SECRET`, `PUBLIC_HOSTNAME` if used, webhook URL token) are replaced by `<REDACTED>` placeholders; structural outcomes (status strings, log-line shapes, row values, route names, prompt versions, latency magnitudes, cited URLs) are preserved verbatim. Before committing, run the redaction grep:

   ```bash
   grep -nE "(sk-[A-Za-z0-9]|tvly-[A-Za-z0-9]|bot[0-9]+:|TELEGRAM_WEBHOOK_SECRET=[^<])" \
     docs/routed-chat-drill/routed-chat-smoke-*-evidence.json && echo "REDACTION FAILED" || echo "redaction grep clean"
   ```
...
`expected_handles` entries in `eval/retrieval/gold.json` use the form `"{external_message_id}#{event_index}"` where `event_index` is the 0-based ordinal of the produced `EventChunk` within the source message after `DomainService.ingest` chunks it. Since one explicit `/note` is exactly one `EventChunk` (I-5 / D-106), `event_index` is always `0` — each corpus message resolves to a single `"#0"` handle whose chunk holds the whole note body. This is internal to the harness only — it is **not** a business event id, **not** a Telegram message id, **not** any external domain identifier. It exists because `chunk_id` is uuid4 at ingest time. The same contract applies unchanged to the OP-5 observability set below.
...
#### `/drafts` author display (D-086 / D-098)
`/drafts` blocks render the author as a resolved display name — the same adapter-only D-086 ladder `/sources` uses (`@username → first_name → opaque `user-<last8>` floor`), requester-`community_id`-scoped (D-089) — instead of the raw opaque `author_user_id`. The block header is `📝 <created_at date> · <author>` (date-only `YYYY-MM-DD`, the full datetime + timezone suffix dropped — D-104) then a blank line then the verbatim draft text; the earlier `author:<id> · id:<short>` technical segments are dropped. Resolution stays adapter-side via the internal helper `adapters/telegram/author_display._resolve_source_author_display`: a draft is already a community-scoped `SourceMessage`, so the snapshot is read directly by its `(external_chat_id, external_message_id, edit_seq)` tuple — **no** `get_source_message` bridge — and a missing / withheld snapshot falls through to the opaque floor (never blank, never a raise). Authorship is still carried only as the opaque `author_user_id` (I-6); the display name is non-authoritative presentation. This makes `/drafts` consistent with the cited-only `/sources` surface — the D-098 "evidence-faithful answer & source attribution" milestone.
...
- **Inspect the recovered cluster.** It listens on loopback-only `RESTORE_PORT` (default `5433`), off the live `5432`; never expose recovered clusters on external VPS interfaces. Verify raw rows with `docker compose exec pg_restore psql -U postgres -d memory_rag -c "SELECT count(*) FROM source_messages;"`.
...
The app port is bound to `127.0.0.1` on the VPS host only. DEPLOY-1.3 (D-062) fronts the app with Caddy and adds the public DNS + HTTPS surface; the loopback publish is retained as an operator-only bypass-the-proxy inspection path. The Postgres host publish is also loopback-only: application containers, `app_init`, and backup sidecars must reach it over the Compose network (`POSTGRES_HOST=postgres`), while operator SQL inspection can use `127.0.0.1:${POSTGRES_PORT:-5432}` from the VPS host. A VPS must not expose `5432/tcp`; after deploy, `ss -ltnp | grep 5432` should show `127.0.0.1:5432` rather than `0.0.0.0:5432`, and an external port probe should not see the service. Off-box backup wiring lands in DEPLOY-1.6 — the archive volume `memory_rag_pg_archive` and the OP-4 base / WAL primitives are unchanged from OP-4. The compose-level `STORAGE_BACKEND=postgres` and `POSTGRES_HOST=postgres` overrides on both `app_init` and `app` ensure the VPS contour boots against the real Postgres backend regardless of operator `.env` defaults; the migrations runner is idempotent, so re-issuing `up -d --build` re-runs `app_init` and exits 0 a second time without touching a head-already database.
