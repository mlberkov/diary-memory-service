...
## Domain semantics
*A-10 → D-114 (edit/delete contract: supersession + tombstone + re-embed; decomposed in `docs/EDIT-DELETE-ROADMAP.md`).*
...
- **A-15. Visibility scopes**: enumerated values for `visibility_scope` are undefined. **Sequenced by D-093 after the first grouped-diary pilot** — community-level scoping (every chat member reads the community corpus) is the access model until then, and Slice 8.2 stays blocked on this. **Distinct from subject scoping (A-45 → D-097):** subject scope is *what a note is about*; visibility is *who may see it* — D-097 does not advance or enumerate A-15. Required before Phase 8 / Slice 8.2.
...
- **A-20. Export/delete semantics**: directionally answered by D-027 for the export half — raw export on demand in JSON or TXT, scope-bounded. Remaining open: delivery channel per host (Telegram file reply / HTTP download / host-app screen), request shape, and the deletion-delivery half (the underlying edit/delete strategy is now ratified in D-114; the user-facing deletion request/delivery semantics remain open here).
...
- **A-28. Mock `/note` accepts ISO-only dates**: the date parser in `core/domain/parser.py` recognizes only `YYYY-MM-DD` on the first non-empty line. Precursor to A-12 (date parsing scope). The only heuristic auto-route that consumed this strict parser (plain-text NOTE detection) is retired by D-078. The `/note`-without-explicit-date → "today" companion landed in D-085: on the explicit `/note` path, a first non-empty line that is not a recognized date now defaults the note to today (`InboundMessage.received_at.date()`) instead of returning `INVALID_INPUT`; the `INVALID_INPUT` reply is now reachable only for an empty/whitespace-only `/note` payload. A-28 stays **open** because the parser strictness itself is unchanged (the default lives in the dispatcher seam, not the parser) — it remains the A-12 precursor.
*A-29 → D-025.*
*A-30 → D-023.*
...
## Routed chat (opened by D-108)
- **A-46. Rewriter subject vocabulary**: the RC-3 query-rewrite contract (`core/chat/rewrite.py`, `QueryRewrite.subject_scope`) carries a seam-ready subject slot per the D-108 "rewrite onto `date_range` / `subject_scope`" clause, but **no RC-3 adapter emits one** — `subject_id` values are opaque past the adapter edge (D-097), no subject-name registry or vocabulary exists anywhere to map natural-language mentions onto, the default adapter mapping is single-subject-per-community → community-wide → `None`, and a fabricated scope under strict matching (D-107) would exclude every chunk. The caller-provided `subject_scope` kwarg passes through to the enrichment retrieval unchanged, and the routed service never applies a rewriter-emitted subject even if a future adapter misbehaves (test-asserted). Required before any packet that wants natural-language subject scoping: a subject vocabulary/registry decision must come first.
- **A-47. Knowledge-search defaults (RC-4 / D-111)**: the knowledge contour ships with tuning placeholders, not quality claims — `knowledge_max_results` defaults to **5**; the Tavily request body is **minimal** (`query` + `max_results` only — no `search_depth`, topic, recency, or domain include/exclude parameters); and `KnowledgeExcerpt.ref` **is the result URL** under the Tavily adapter (the core port keeps `ref` an opaque locator, so a curated provider may use any stable reference string). None of these were measured against retrieval/answer quality; a future quality packet (or the first curated-provider packet) revisits them with evidence. The operator drill observes them as-is.

## Author display (opened by D-081)
- **A-44. Author display-name resolution**: human-readable author display is resolved **only at the Telegram adapter seam** from host-supplied identity fields, with the fallback chain `username → first_name → opaque short-ID`. The canonical core identifier stays the opaque `author_user_id` (I-6); the core never decodes or renders it. Resolved names are host-supplied and **non-authoritative** (a Telegram user may change or withhold `username` / `first_name`) — presentation, not identity, never a substitute for `author_user_id` in storage, retrieval, scoping, or provenance. The single sanctioned display surface this milestone is `/sources` (D-036); answer-reply (`/ask` reply) attribution is deferred to a later named packet. This does **not** change A-15 visibility semantics (it governs who-authored display, not who-may-see). Opened by D-081. D-082 pins the **capture / persistence shape** (snapshot-oriented, adapter/storage-owned, nullable, non-authoritative). D-083 pins the **landing seam** (Option A): a separate Telegram-adapter-owned side table written through an adapter-owned storage port distinct from the core `DomainRepository`, keyed by the message idempotency tuple `external_chat_id + external_message_id + edit_seq` as opaque scalars; the snapshot never enters a core type. D-084 **implements** that capture + durable landing: `TelegramUser` carries nullable `username` / `first_name`, the webhook snapshots them on source-message-bearing routes (note/draft) straight from `message.from_`, and they land in the adapter-owned `author_display_inputs` side table via the co-located `AuthorDisplayInputStore` port (mock / sqlite / postgres parity; R-2 idempotency; both-null still recorded). D-086 **implements resolution + `/sources` rendering**: the adapter resolver applies the fallback chain (`@username → first_name → opaque `user-<last8>` floor`) over the captured snapshot, bridging the opaque boundary via `get_source_message` to recover the keying tuple; `/sources` renders a `— <author>` line per block, resolution staying adapter-only. **A-44 is resolved by D-086** (resolution + `/sources` rendering — its open condition — delivered). Answer-reply (`/ask` reply) attribution remains a **separate** deferred placeholder, not a gate on this closure. **D-091** now pins the *contract* for that separate `/ask`-reply surface (contributor attribution over the answer's grounding chunks: distinct authors by opaque `author_user_id`, labeled `Contributors: …` footer, adapter-resolved via the same D-086 seam, requester-`community_id`-scoped per D-089), with the rendering deferred to its follow-up code packet — A-44 stays **resolved by D-086** and is not reopened.
...
- A-44 → D-086 (author display-name resolution + `/sources` rendering — the adapter-only fallback chain `@username → first_name → opaque `user-<last8>` floor` over the D-084 snapshot, bridging the opaque boundary via `get_source_message`; `/sources` renders a `— <author>` line per block; resolution stays adapter-only, names non-authoritative, core carries only opaque `author_user_id`. Answer-reply / `/ask`-reply attribution remains a separate deferred placeholder, not part of this closure).
- A-43 → D-077 (DEPLOY-1 logs-first observability scope pinned to the existing `pg_backup.*` log-prefix family in `scripts/pg_backup/scheduler.sh` + `scripts/installer/deploy.sh` + `scripts/pg_offbox_uploader/uploader.sh`, Caddy access logs from the reverse-proxy contour, and the app-side `telegram.webhook` / `retrieval.hybrid` / `answer.*` log families already emitted by `src/memory_rag/adapters/telegram/webhook.py` / `services/retrieval.py` / `services/query_service.py` / `services/dispatcher.py` — captured verbatim, not invented. No new logging contract in `src/`. Forward seam to remote sinks deliberately unpinned and deferred to a later observability packet or DEPLOY-2 prep).
- A-14 → D-093 (community-bootstrap half: implicit-on-first-message bootstrap ratified; the 1:1 `external_chat_id → community_id` adapter-axis mapping ratified — no longer a "surrogate"; membership inherited from host-chat membership with no core ACL; multi-diary-on-one-instance ratified as a core capability. The **subject/child-bootstrap half is carved out to A-45**; `/setup` is deferred-optional; A-15 visibility deferred and sequenced after the first grouped pilot. Sequenced docs-first in `docs/GROUPED-MULTI-DIARY-ROADMAP.md`).
- A-45 → D-097 (subject-scoping bootstrap/assignment contract: an opaque / community-scoped / **nullable** `subject_id` on `Note` / `EventChunk`, born directly as `subject_id` — `child` stays a use-case label; assignment is an adapter-axis function with a **default single-subject mapping**; `null` = community-wide; **no** core subject registry/entity; an **optional** retrieval filter mirroring the D-040 `date_range` seam; **separate from A-15 visibility**. Resolved at the contract level only; the code realization has since landed as Milestone H / H-1..H-4 in `docs/SUBJECT-SCOPING-ROADMAP.md` (milestone closed)).
