...
Answer-reply contributor attribution (D-091, contract only — not yet surfaced): the `feat/ask-reply-contributor-attribution` milestone pins, by contract, that the `/ask` grounded reply will carry a `Contributors: …` footer line beneath `answer_text` naming the **distinct contributors who grounded the answer** — the distinct authors (by opaque `author_user_id`) of the answer's grounding chunks (the same chunks `/sources` recalls), deduped by `author_user_id` and listed in first-appearance order, rendered only when the grounding set is non-empty. It reuses the same adapter-only resolver (`resolve_chunk_author_display`, requester-`community_id`-scoped per D-089) and the D-086 fallback chain, so the same non-authoritative / opaque-floor semantics apply. This is **recorded by D-091 but not surfaced in `/ask` replies** until D-091's follow-up code packet lands; today the `/ask` reply is still `answer_text` alone (D-069).
