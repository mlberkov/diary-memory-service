...
## D-108 — RC-1: routed-chat contract (route taxonomy, provenance model, control surface, provider decisions) + owner-ordered A-10 re-queue (docs-first)

### Context

The owner has handed over an authoritative milestone spec for a **routed conversational entry point**: an incoming natural-language question is classified into one of four routes and answered accordingly, combining the existing note-grounded answering with model-knowledge answering and web-grounded answering under explicit per-source provenance labeling — surfaced this milestone only through the existing Telegram control surface, with the same core seam intended for later hosts (Phase 9 / A-21 untouched and undecided). Current state the contract builds on: one grounded `/ask` flow (I-9; D-033 citation grounding, D-035 fallback grading, D-099 no-evidence guardrail); the OpenAI provider contour (D-024 embeddings, D-037 canonical `chat_model = "gpt-4.1"` boot-gated, D-047 / D-049 timeout-retry-backoff hardening via `adapters/resilience.py`); the optional retrieval kwargs `date_range` (D-040) and `subject_scope` (D-097 / D-107) at the service seam; the trace plane (`Query` / `RetrievalHit` / `AnswerTrace` — D-032 / D-035; R-5 / R-6 / R-7 / R-11); the Stage-2 gate **open** (OP-1..OP-5 complete, D-043 / D-044); REAL-1 closed (D-073 / D-074).

This selection is an **explicit owner override of the default pick-next sequencing**: the edit/delete milestone (A-10, TechSpec §12, Slice 2.5) — the previously recommended next milestone — is **re-queued as the milestone immediately after this one**. The override is recorded here in the same register as the prior owner sequencing decisions (D-060 hosting-order re-sequence, D-073 binding owner rule, D-093 grouped-diary prioritization, D-103 cleanup sequenced ahead of the in-flight Milestone H); the D-043 gate discipline is not violated — the gate is open, and this milestone is sequenced separately from the Phase/Stage axis. Owner rationale: routed chat is the highest-value near-term capability for daily users; it has no dependency on A-10; and the interplay is favorable — routing reuses the existing retrieval seam unchanged, so when A-10 lands later, tombstoned/superseded exclusion (R-4) applies to routed answers automatically.

This is the docs-first opener (Packet RC-1). It mirrors the D-093 / D-097 / D-106 precedent: this entry carries the stable contract; the new `docs/ROUTED-CHAT-ROADMAP.md` carries the refinable RC-1..RC-4 sequence. It changes no `src/`, schema, or runtime behavior.

### Decision

- **Owner sequencing override + A-10 re-queue (owner-fixed).** The routed-chat milestone is next; **A-10 edit/delete is the milestone immediately after** (visible in `docs/todo.md` and on the execution-map 2.5 row). A-10 is re-queued, not advanced: no edit/delete mechanics are decided here.

- **Route taxonomy (owner-fixed, verbatim).** Four routes, classified per inbound question:
  - `diary_lookup` — answer source: diary retrieval only (the existing grounded ask, unchanged). Example: "When did he say his first word?"
  - `diary_plus_llm` — answer source: LLM generation with diary context (enrichment retrieval). Example: "What games would suit our son right now?"
  - `diary_plus_web` — answer source: web search + diary context, both cited. Example: "Why doesn't a child pronounce 'r'?"
  - `general_llm` — answer source: LLM directly, no retrieval. Example: "What is phonemic awareness?"

  Pipeline per route: classify → (rewrite query → scoped retrieval) → (web search where applicable) → generate with provenance labels. Enrichment pattern for `diary_plus_llm` / `diary_plus_web`: retrieve personal context first (age, recurring topics, relevant facts), then rewrite the outward query using that context, then answer combining both planes.

- **Dual-register route naming (decided).** The four names above are the **product-register** labels (owner spec, authoritative in owner-facing prose and product docs). `diary` is use-case vocabulary and must not enter core identifiers (D-026 / D-041; `docs/GLOSSARY.md`), so — per the D-097 `child` → `subject_id` precedent — the core route identifiers RC-2 is born with are fixed now as the canonical mapping: `diary_lookup` → **`notes_lookup`**, `diary_plus_llm` → **`notes_plus_model`**, `diary_plus_web` → **`notes_plus_knowledge`** (named for the knowledge-source seam, not welded to "web"), `general_llm` → **`model_only`**. Same taxonomy, two registers; the mapping is recorded in `docs/GLOSSARY.md`.

- **Architectural placement (ratified).** The router lives in the **functional core at the service seam** (same plane as the current ask flow), not in the Telegram adapter (I-1). Telegram remains an event-source / control-surface adapter that forwards the question and renders the labeled answer, calling in-process exactly like today's command handling. Dispatch is **hand-rolled** — single-LLM classification + dispatch, not multi-agent orchestration; the framework non-goal stands (no LangGraph et al.). A-21 / Phase 9 (HTTP API vs SDK) is neither opened nor pre-decided; the seam is designed so Phase 9 can expose it without reshaping.

- **Control surface (decided): new `/chat` command.** Routed chat arrives as a new explicit command, not an extension of `/ask`. Why: `/ask`'s deterministic grounded contract (and its byte-pinned reply wording) stays structurally untouched rather than guaranteed-by-care; D-027 / D-030 lifecycle vocabulary reserves `/ask` for the query lifecycle, and the repo's established seam for a new user action is a new command (D-028 / D-029 / D-036). Hard requirement either way, kept: existing `/note` / `/ask` / `/drafts` / `/export` / `/sources` behavior unchanged; the unconditional draft floor (I-14 / R-13) untouched.

- **Classifier provider (decided).** Classification is LLM function-calling on a light model behind a provider adapter following the existing hardening pattern (explicit timeouts, bounded retries, trace capture — D-047 / D-049, R-7 / R-9). Provider: **OpenAI** (the only landed provider contour); canonical classifier pin: **`gpt-4.1-mini`** — a separate canonical pin alongside (not a change to) the D-037 `chat_model = "gpt-4.1"` boot gate. The `Settings` knob and the boot-gate clause land with RC-2.

- **Web-search / knowledge-source seam (decided).** The knowledge-source side of `diary_plus_web` sits behind a **narrow core interface** (a knowledge-source port, provider-SDK-free per I-11) — a curated domain-knowledge provider is expected later as a second source alongside/instead of generic web, so the route is not welded to "web". One new hardened provider adapter implements it, following the same D-047 / D-049 pattern; R-7 already names "search backend" in provider-call observability and R-9 bounds apply. Provider: **Tavily**.

- **Provenance model — I-9 generalized, not violated (ratified).** I-9 is preserved verbatim for the notes-grounded class and generalized for routed answers: **every answer segment carries a provenance class** — **notes** (cited via the existing cited-only evidence surface, D-098 / D-099 / D-100), **web** (cited URLs), or **model** (explicitly labeled as model knowledge). **No segment may present model or web content as if it came from the diary.** Recorded as an appended I-9 clause in `docs/INVARIANTS.md`; enforcement lands with RC-2..RC-4.

- **Scoping everywhere (ratified).** R-3 / R-8 community scoping — and the optional `subject_scope` dimension — apply to **every retrieval and every prompt in every route**, including the enrichment retrievals inside `diary_plus_llm` / `diary_plus_web`. Test-asserted by the RC-3 / RC-4 suites.

- **Medical non-goal amended + escalation prompt invariant (owner-fixed).** The PRD out-of-scope bullet "medical recommendations" is amended explicitly, not silently violated: the system **may** give general developmental information and activity suggestions in reactive answers; it must **never** produce a diagnosis or symptom interpretation; and whenever the question or the retrieved diary context suggests a potential developmental/medical red flag, the answer **must recommend consulting a specialist**. The escalation rule ships as a system-prompt invariant of routes `diary_plus_llm` / `diary_plus_web` and is covered by tests on red-flag-style prompts (mock providers) in the code packets. The amendment does not relax the adjacent "automatic developmental coaching" / "proactive recommendations" exclusions.

- **Failure / fallback policy (ratified).** **No numeric confidence thresholds** — function-calling classifiers do not return calibrated confidence, and none are fabricated. On classification failure or ambiguity the dispatcher defaults to `diary_lookup` plus honest-degradation wording. Honest degradation: if diary retrieval returns nothing above the existing fallback grading (`FallbackMode`, D-035), the answer states this explicitly **before** any general/web content ("no diary evidence on this — here is general information…"). Requested vs effective route stays distinguishable per R-6; routing accuracy is observed via traces, not numerically gated in this milestone.

- **Trace contract (ratified, contract level).** Routing decisions, classifier outputs, rewritten queries, and retrieval and web traces are captured in the existing trace plane (the `Query` / `RetrievalHit` / `AnswerTrace` lineage — D-032 / D-035; R-5 / R-7 / R-11 discipline). Schema extensions are **additive and non-destructive** and land with the code packets that make them real; no confidence column exists (per the no-thresholds rule).

- **Query rewriting reuses landed capabilities (ratified).** Rewriting maps the user question onto the already-shipped keyword-only retrieval kwargs at the service seam — `date_range` (D-040) and `subject_scope` (D-107). **Side effect recorded:** this partially delivers the open inbound `/ask` filter-syntax follow-up (`docs/todo.md`, Slice 3.4 (cont.)) on its natural-language side; the explicit-syntax half remains a separate, open item. Per I-10, routed chat is a new explicit, disableable surface: the base `/note` → `/ask` flow works end-to-end with routed chat absent or disabled (the Phase 5.2 "query rewriting behind a flag" row is related but not merged into this milestone).

- **Untouched guarantees.** No destructive schema changes; raw-before-enrichment (I-3 / R-1) untouched; PostgreSQL stays the sole durable source of truth (I-2).

### Why

The owner ordered the milestone by user value, and the contract-before-code discipline (D-093 / D-097 / D-106) demands the route taxonomy, provenance model, and provider decisions be canonical before any dispatcher exists. A new `/chat` command keeps the deterministic `/ask` contract structurally intact instead of relying on a classifier to preserve it. Same-provider classification (`gpt-4.1-mini`) extends the one landed, hardened provider contour instead of introducing a second provider for a light task — mirroring the D-024 / D-037 single-canonical-pin discipline. A knowledge-source interface (rather than a web-search interface) keeps `diary_plus_web` open to the expected curated domain-knowledge provider without a later reshaping. Refusing numeric thresholds keeps the fallback policy honest about what function-calling classifiers actually return; defaulting ambiguity to `diary_lookup` fails toward the grounded, already-guaranteed route. Generalizing I-9 by provenance class extends "every answer is grounded in retrieved evidence" to "every answer segment declares its evidence plane" without weakening the notes-grounded guarantee that exists today.

### Consequence

- **Docs (this packet):** this D-108 entry; new `docs/ROUTED-CHAT-ROADMAP.md` (refinable RC-1..RC-4 packet ladder + milestone exit criteria); `docs/INVARIANTS.md` (I-9 appended provenance-class clause, marked docs-first / enforced by RC-2..RC-4); `docs/product/PRD.md` (§6 medical bullet amended); `docs/product/TechSpec.md` §4 (target command surface gains the `/chat` line, marked "lands with RC-2"); `docs/GLOSSARY.md` (route dual-register mapping); `docs/assumptions.md` + `docs/assumption-audit.md` (A-10 re-queue annotations — A-10 stays open, no new assumption number); `docs/execution-map.md` (new "Routed chat core" block + 2.5-row re-queue annotation); `docs/todo.md` (RC milestone block + A-10 queued-next block + filter-syntax side-effect note).
- **No `src/` / `tests/` / schema / DDL / migration / config change.** No new I-/R- number; no runtime behavior change. **No claim of a `/chat` command, route enum, classifier knob, web adapter, or trace field is made until RC-2..RC-4 make each true** (mirroring D-097's "no `src/` claim until H-1"). `docs/RUNTIME-INVARIANTS.md` is deliberately untouched: R-7 already names the search backend, R-3 / R-8 / R-6 / R-11 already carry the scoping and honesty contracts, and new routed-chat runtime clauses become true only when the code packets land (D-073 / D-097 precedent). `[[feedback_minimal_packet_docs]]`, `[[feedback_doc_state_truthfulness]]`, `[[feedback_full_gate_and_doc_truthfulness]]`.
- **Sequenced code work (not in this packet):** RC-2 — classifier adapter (`gpt-4.1-mini` pin + knob + boot-gate clause), hand-rolled dispatcher at the service seam, routes `diary_lookup` (delegating to the existing grounded ask unchanged) + `general_llm` (direct LLM, model-labeled), Telegram `/chat` wiring, routing captured in traces. RC-3 — enrichment retrieval + rewrite-to-kwargs (`date_range` / `subject_scope`), `diary_plus_llm` with mixed notes+model labeling, regression coverage asserting R-3 / R-8 scoping inside enrichment. RC-4 — hardened Tavily knowledge-source adapter, `diary_plus_web` full pipeline (enrich → rewrite → search → synthesize, citing both planes), escalation invariant verified on red-flag-style prompts (mock provider), consolidated regression suite (mock + PG-gated), RUNBOOK operator section, doc reconciliation, milestone closure.

### Out of scope (per packet boundaries)

- Any `src/` / `tests/` / schema / migration / config change — including the `/chat` command, route enum, classifier, dispatcher, rewriting, enrichment, web search, and trace fields (RC-2..RC-4).
- Edit/delete mechanics (A-10) — explicitly the **next milestone after this one**, re-queued, not advanced.
- A-21 / Phase 9 host-integration seam work (HTTP API / SDK), web UI, theygrow.app embedding.
- Thematic-tag enrichment at ingestion; TheyGrow knowledge-graph / ontology integration; memory synthesis / proactive summaries; voice/photo ingestion; multi-agent orchestration.
- The explicit `/ask` filter-syntax item (stays open; only its natural-language side is partially delivered by RC-3).
- The curated domain-knowledge provider (expected later behind the same knowledge-source seam; not implemented or decided beyond the seam shape).

---

## D-109 — RC-2: classifier + dispatcher + two routes (`notes_lookup` / `model_only`, Telegram `/chat`, routing traces)

### Context

Packet RC-2 of the routed-chat milestone (D-108; `docs/ROUTED-CHAT-ROADMAP.md`). RC-1 ratified the contract docs-first; this packet is the first code packet: it lands the routed entry at the service seam, the classifier provider contour, the two no-enrichment routes, the `/chat` control-surface wiring, and the routing-trace capture. Every design decision below implements a clause already fixed by D-108; this entry records the implementation-level choices RC-2 made inside that contract.

### Decision

- **Core route taxonomy (`src/memory_rag/core/chat/`).** New SDK-free core package, separate from `core/routing/` (which owns the message-level `RouteKind` plane): `ChatRoute` StrEnum with the four canonical identifiers (`notes_lookup` / `notes_plus_model` / `notes_plus_knowledge` / `model_only`); `RouteClassification` (no confidence field, per the D-108 no-thresholds rule); `ChatRouteDecision` (the persisted routing trace); `RoutedChatResult` (wrapping the existing `AnswerResult`); the `ChatRouteClassifier` Protocol with a two-way failure split mirroring D-035 — `ChatRouteClassifierUnavailableError` (no output existed) vs `ChatRouteOutputError` (output existed but unusable, carried verbatim).
- **Classifier provider contour.** New adapter package `adapters/chat_routing/` cloning the `adapters/answers/` pattern: factory switching on the new `Settings.classifier_backend` (`mock` default), deterministic in-band-steerable `MockRouteClassifier`, and `OpenAIRouteClassifier` using **function calling** (forced `select_route` tool whose `route` parameter is an enum of the four canonical values, `temperature=0`) under the D-047/D-049 hardening (`run_with_retries`, per-attempt timeout, `max_retries=0` on the SDK client). The retry policy **reuses the shared `provider_*` knobs** — no third knob set. Boot gate: `_verify_classifier_contour` with the canonical pin `gpt-4.1-mini` — a separate pin alongside, not a change to, the D-037 `gpt-4.1` chat gate.
- **Routed service (`services/routed_chat.py`).** `RoutedChatService.chat(message, *, subject_scope=None)`: hand-rolled classify-then-dispatch. One branch funnels classifier failure, unusable output, the not-yet-dispatchable routes, and the empty question to the default `notes_lookup` (D-108 fallback policy); requested vs effective route is preserved on the decision row, the result, the dispatch metadata, and a `chat.routed` log line (R-6 / R-11). `notes_lookup` delegates to `QueryService.answer` **unchanged** (its internals are untouched by this packet), including the dispatcher-style `NotImplementedError` handling for retrieval-less backends.
- **`model_only` answering.** Reuses the existing `ChatClient` port — generation stays on the D-037 `gpt-4.1` contour; the classifier pin is classification-only. New core contract `core/chat/model_prompt.py`: `build_model_only_prompt` (prompt version **`model-only-v1`**, JSON-object response shape because the OpenAI chat adapter hardwires `response_format={"type":"json_object"}`, system text forbidding notes attribution) and a **lenient** `parse_model_only_answer` (string `answer_text`, extra keys ignored). Failure grading mirrors D-035: `PROVIDER_UNAVAILABLE` (empty trace answer, zero latency/tokens) and `PARSE_FAILURE` (trace preserves `raw_text` verbatim). The path writes its **own** `Query` row (`model_name` = the generation chat model, zero retrieval hits) plus `AnswerTrace` (`prompt_version="model-only-v1"`, empty `context_chunk_ids`) so R-5 stays uniform across routes. `MockChatClient` gained an additive branch keyed on the new prompt version; the v1 contour is byte-unchanged.
- **Routing trace = new `chat_route_decisions` table (migration `0007.chat-route-decisions`), not columns on `queries`.** One row per `/chat` call: `requested_route` (nullable; NULL = no usable classification), `effective_route`, classifier provenance (model name, verbatim raw output, bounded-loop latency), own `community_id` (fail-closed scoped reads without a parent join), nullable `query_id` link (NULL only when the delegated retrieval seam was unavailable before a `Query` row existed). Why a separate table: the `notes_lookup` `Query` row is written inside `QueryService._finalize`, which this packet must not touch — additive columns there would force a core-service change or a post-hoc UPDATE; a new table keeps the change strictly additive and the existing rows untouched. No confidence column exists. Repository seam: `save_chat_route_decision` / `get_chat_route_decision` on all three backends (mock, SQLite inline-DDL, Postgres).
- **Control surface.** `RouteKind.CHAT` + the `"/chat"` command token (lifecycle stays `"other"`; the `"query"` lifecycle remains the D-027/D-030 `/ask` lifecycle). `Dispatcher` gains a keyword-only `routed_chat: RoutedChatService | None = None` dependency — routed chat is an explicit, disableable surface (I-10): with it absent, `/chat` returns a fixed cause-neutral pointer to `/ask` and the base `/note` → `/ask` flow is untouched. Reply formatting: the body always goes through the existing `_format_answer_reply`, so an effective-`notes_lookup` `/chat` reply is **byte-identical** to the `/ask` reply for the same outcome (one formatting surface; D-099 guardrail inherited); a rerouted answer appends one cause-neutral trailer `(answered from your saved notes)`; a successful `model_only` answer appends `(model knowledge — not from your saved notes)` (generalized I-9: model content is never presented as note-grounded); `model_only` failures reuse the pinned provider/parse literals. **One deliberate pinned-literal change:** `_REPLY_HELP` now lists `/chat` (it claims to enumerate "Commands:"); its byte pin moved with it, and every sibling literal is asserted byte-identical.
- **`/sources` is deliberately not fed by `/chat`.** The cache, its replies, and its pins remain the `/ask`-only surface they describe (D-036 / D-099); test-asserted in both directions (no entry created; the `/ask` set not overwritten). Revisit with RC-3/RC-4 if the product wants routed answers in `/sources`.

### Why

A separate decision table keeps `QueryService` byte-identical while still letting one SQL join (`queries` ⟕ `answer_traces` ⟕ `chat_route_decisions`) inspect every routed answer — R-5 uniformity without touching the landed trace writers. Reusing `_format_answer_reply` prevents a second, divergently-worded grounded-answer surface from ever existing. A single cause-neutral funnel branch (and one reroute trailer) keeps user-facing wording honest without naming causes the user cannot act on; R-6 carries the distinction in traces and metadata instead. The lenient model-only parser accepts the structured-answer shape a provider may volunteer, so the route degrades by labeling, not by brittleness. The keyword-only dispatcher dependency keeps the 13 existing positional `Dispatcher(...)` constructions valid and routed chat disableable per I-10.

### Consequence

- **Code:** new `core/chat/` package; new `adapters/chat_routing/` package; new `services/routed_chat.py`; migration `0007.chat-route-decisions.sql`; repository protocol + mock/SQLite/Postgres implementations; `RouteKind.CHAT` + `/chat` token; dispatcher CHAT branch + trailers + updated `_REPLY_HELP`; webhook composition-root wiring; `Settings.classifier_backend` / `Settings.classifier_model`; `_verify_classifier_contour` boot gate; additive `MockChatClient` model-only branch; one-line `Query.model_name` docstring amendment.
- **Tests:** new modules `test_model_only_prompt.py`, `test_chat_route_classifier_mock.py`, `test_openai_route_classifier_retry.py` (offline, injected fake client, function-calling shape asserted), `test_boot_classifier_gate.py`, `test_storage_chat_route_decisions.py` (mock + sqlite + PG-gated), `test_routed_chat_service.py`, `test_dispatcher_chat.py`; extended `test_chat_client_mock.py`, `test_config.py`, `test_postgres_migrations.py` (`MIGRATION_COUNT = 7`, 0007 upgrade-preserves-data, stagers fixed to keep the new tail pending), `test_telegram_reply.py` (updated `_REPLY_HELP` pin; sibling pins unchanged), `test_telegram_commands.py`, `test_routing_classifier.py` (plain text never routes to CHAT — `/chat` is command-only this milestone), `test_end_to_end_smoke.py` (per-route `/chat` round-trips; `/ask` reply unchanged by the wiring).
- **Known asymmetry (restated from D-107):** pre-existing local SQLite files do not gain `chat_route_decisions` (SQLite schema is inline DDL on fresh files; there is no SQLite migration runner); Postgres is the canonical durable backend and migrates via 0007.
- **Untouched:** `QueryService.answer` internals; every reply literal except `_REPLY_HELP`; the unconditional draft floor (I-14 / R-13); the heuristic plain-text classifier; raw-before-enrichment (I-3 / R-1); Postgres as sole durable source of truth (I-2). No new I-/R- number.

### Out of scope (per packet boundaries)

- Enrichment retrieval, query rewriting, and `diary_plus_llm` behavior beyond enum/fallback scaffolding (RC-3).
- Knowledge-source port, Tavily adapter, and `diary_plus_web` behavior beyond enum/fallback scaffolding (RC-4).
- Red-flag escalation prompt invariant and tests (a property of the RC-3/RC-4 mixed routes).
- `/sources` integration for `/chat` answers; honest-degradation wording preceding general content (RC-3).
- RUNBOOK operator section, REAL round-trip evidence, milestone-closure flips (RC-4); A-10 edit/delete (next milestone).

---

## D-110 — RC-3: enrichment + rewriting + `notes_plus_model` (`diary_plus_llm`)

### Context

Packet RC-3 of the routed-chat milestone (D-108; `docs/ROUTED-CHAT-ROADMAP.md`). RC-2 (D-109) landed the classifier/dispatcher seam and the two no-enrichment routes, leaving `notes_plus_model` / `notes_plus_knowledge` as enum members funnelled to `notes_lookup`. This packet makes the first mixed route real: classify → rewrite-to-kwargs → scoped enrichment retrieval → one generation combining a citation-grounded notes segment with an explicitly labeled model-knowledge segment (generalized I-9). Every design decision below implements a clause already fixed by D-108; this entry records the implementation-level choices RC-3 made inside that contract.

### Decision

- **Rewrite is retrieval-side only.** D-108 describes two rewrite phrasings: "classify → (rewrite query → scoped retrieval)" and "retrieve personal context first, then rewrite the outward query using it". The outward-query rewrite (conditioning an external search on retrieved personal context) is the RC-4 knowledge-source step — `notes_plus_model` makes no outward call. RC-3 delivers the retrieval-side rewrite-to-kwargs the roadmap RC-3 row names: a natural-language question maps onto the landed keyword-only kwargs `date_range` (D-040) / `subject_scope` (D-107); the `QueryService.answer` seam signature is untouched. The generation prompt receives the **original** question — the rewritten query exists for retrieval and the trace plane only.
- **Rewriter contract + adapters mirror the classifier.** New SDK-free core contract `core/chat/rewrite.py`: `QueryRewrite` (non-empty `retrieval_query`, optional `DateRange`, seam-ready `subject_scope` slot, verbatim `raw_output`, model name, bounded-loop latency), `QueryRewriter` Protocol with keyword-only `today: date` (the caller supplies the anchor for relative expressions — the contract stays deterministic given inputs), and the D-035-mirroring failure split `QueryRewriterUnavailableError` (no output) vs `QueryRewriteOutputError` (unusable output, carried verbatim — including empty `retrieval_query`, unparseable ISO dates, and a contradictory range). Adapters in `adapters/chat_routing/`: deterministic constructor-steerable `MockQueryRewriter` (identity rewrite by default) and `OpenAIQueryRewriter` (forced `rewrite_query` function calling — required `retrieval_query`, optional ISO `date_from`/`date_to`, **no subject parameter** — `temperature=0`, D-047/D-049 hardening, today's date embedded in the system text).
- **The rewriter rides the classifier contour — no new knobs, no new pin.** `build_query_rewriter` switches on the existing `classifier_backend`, uses `classifier_model` (canonical `gpt-4.1-mini`) and the shared `provider_*` retry knobs (the D-109 "no third knob set" discipline). D-108 pins only the classifier; this entry records the contour reuse as an implementation-level decision, not a change to the pin set. `_verify_classifier_contour` additionally builds the rewriter through the same factory (boot gate and request path cannot disagree); no new gate clause exists because no new knob exists.
- **Subject slot is seam-ready but never emitted (A-46).** Subject ids are opaque past the adapter edge (D-097), no subject-name vocabulary exists, the default mapping is community-wide `None`, and a fabricated scope under strict matching (D-107) would exclude every chunk. The OpenAI tool schema deliberately has no subject parameter; the routed service applies only the **caller-provided** `subject_scope` to the enrichment retrieval even if a future adapter misbehaves (test-asserted); a rewriter-emitted value is still recorded truthfully in the trace row. Recorded as **A-46** in `docs/assumptions.md` / `docs/assumption-audit.md`.
- **Retrieval reuse without moving persistence.** `QueryService` gains a public, **pure** `retrieve(community_id, query_text, *, date_range=None, subject_scope=None) -> RetrievedCandidates` (embed → both legs with R-3/R-8 + kwargs → RRF; writes no rows); `answer()` now calls it on its retrieval step — a behavior-preserving refactor proven by the unmodified `/ask` suites. Hit-row construction moved verbatim to `services/retrieval.py` as `build_retrieval_hits` (with `SPARSE_MODEL_NAME` / `rrf_leg_score`), shared by `QueryService._persist_trace` and the new route. `_normalize_query` became public `normalize_query`. `Query.fallback` remains a single post-generation decision in both services (D-035).
- **`notes-plus-model-v1` prompt contract (`core/chat/enriched_prompt.py`).** `build_notes_plus_model_prompt(context)` enforces R-8 single-community (reusing `CrossCommunityContextError`), renders chunks in the v1 line format (canonical-register placeholder `(no note chunks were retrieved for this question)`), demands a single JSON object (the OpenAI chat adapter hardwires `response_format=json_object`, D-109), and carries the **escalation clause** as a module-level constant `ESCALATION_CLAUSE` — the D-108 medical amendment made live for this route (general developmental information and activity suggestions allowed; never a diagnosis or symptom interpretation; potential red flags ⇒ recommend consulting a qualified specialist). The constant is byte-pinned in tests and RC-4 reuses it verbatim. Response shape: `{"notes_text", "cited_chunk_ids", "model_text", "notes_uncertainty"}` with a **three-valued** marker (`confident`/`uncertain`/`no_evidence` — `ambiguous` is unused on this route, matching what the prompt offers). `parse_notes_plus_model_answer` is strict where honesty is at stake — required fields/types, fabricated citations rejected against the context (I-9), and the consistency triple `no_evidence ⇔ cited_chunk_ids == () ⇔ notes_text == ""` enforced both directions (a notes claim without a citation never renders as note-grounded) — and lenient on extra keys (the RC-2 `parse_model_only_answer` precedent).
- **Route execution (`services/routed_chat.py`).** `RoutedChatService.__init__` gains keyword-only `rewriter: QueryRewriter | None = None` (`None` ⇒ rewrite-unavailable semantics; existing constructions stay valid). The funnel narrows: `notes_plus_model` dispatches; classifier failure, unusable output, `notes_plus_knowledge` (RC-4), and the empty question still funnel cause-neutrally to `notes_lookup`. `_answer_notes_plus_model`: rewrite (failure — unusable output / provider unavailable / no rewriter / rewrite normalizing to empty — degrades cause-neutrally to the normalized original question with no date constraint, logged as `chat.rewrite_failed`; logs may name causes, replies must not) → `QueryService.retrieve` with the caller's `subject_scope` → context assembly (`query_text` = original stripped question) → one `ChatClient.complete` on the D-037 `gpt-4.1` generation contour. **Empty merged retrieval does not short-circuit** — generation still runs (the route's point) and the parser forces the empty notes plane. An enrichment retrieval seam raising `NotImplementedError` is handled like the `notes_lookup` branch (synthetic `NO_EVIDENCE`, `query_id=None`, no rewrite row).
- **Grading (single post-generation decision).** Provider unavailable → `PROVIDER_UNAVAILABLE` (empty trace text, zero tokens/latency); bad JSON / shape / fabricated citation / consistency violation → `PARSE_FAILURE` (trace preserves `raw_text` verbatim); success `confident`/`uncertain`/`no_evidence` → `NONE`/`WEAK_EVIDENCE`/`NO_EVIDENCE` with both segments carried. On the success contours the `AnswerTrace.answer_text` stores the **raw segmented JSON verbatim** — a mixed answer has no single answer string; this is the D-035 truthful-provenance rule applied to a segmented shape, keyed by `prompt_version="notes-plus-model-v1"`. The path writes its own `Query` row (original question, recorded `subject_scope`, embedding model name), `RetrievalHit` rows via `build_retrieval_hits` on every contour that ran retrieval, and one `AnswerTrace`; a `chat.enriched` log line mirrors `retrieval.hybrid` semantics (R-11).
- **`AnswerResult.model_text` carriage (additive).** One optional defaulted field carries the explicitly-model-knowledge segment to the reply layer; `None` on every pre-existing contour, so every existing constructor call stays valid and `RoutedChatResult` is unchanged. `answer_text` keeps its existing meaning (the notes-grounded text; `None` when the notes plane is empty).
- **Rewrite trace = new `chat_query_rewrites` table (migration `0008.chat-query-rewrites`), not columns on `chat_route_decisions`.** One row per `notes_plus_model` execution: nullable `rewritten_query` (`NULL` = no usable rewrite existed, mirroring the `requested_route` NULL pattern), nullable `date_start`/`date_end` (`CHECK (date_start <= date_end)`), nullable `subject_scope` (rewriter-emitted, always NULL this packet — A-46), rewriter provenance (`rewriter_model_name` — `""` only when no rewriter was wired; `rewriter_raw_output` `""` when no output existed, verbatim otherwise; bounded-loop latency), own `community_id` for fail-closed scoped reads, `decision_id` FK + UNIQUE (1:0..1 per decision). Why a new table: the RC-2 reasoning applies verbatim — existing writers untouched, 0007 byte-stable, strictly additive — and the row exists only when the route executed. **Chronology:** the decision row is persisted post-answer, so `chat()` writes the rewrite row strictly after `save_chat_route_decision` (test-asserted). Repository seam: `save_chat_query_rewrite` / `get_chat_query_rewrite_for_decision` on all three backends (mock, SQLite inline-DDL, Postgres).
- **Reply rendering (dispatcher register, zero pre-existing literal changes).** New `_format_notes_plus_model_reply` behind a `NOTES_PLUS_MODEL` branch in `_format_chat_reply` (only an exact requested match dispatches there, so it never carries the reroute trailer). Three new pinned literals — `_HEADER_CHAT_NO_NOTES` ("Nothing in your saved notes covers this — here's general information instead."), `_HEADER_CHAT_WEAK_NOTES` ("Your saved notes only weakly cover this."), `_LABEL_CHAT_NOTES` ("From your saved notes:") — keeping the dispatcher's "your saved notes" register; D-108's "no diary evidence on this — here is general information…" was a wording sketch whose semantics are preserved: the degradation statement renders strictly **before** any model content. The existing `_TRAILER_MODEL_KNOWLEDGE` (byte-unchanged) doubles as the inline model-segment label; an empty model segment is omitted entirely (no orphan label, no unlabeled model content possible); `PROVIDER_UNAVAILABLE`/`PARSE_FAILURE` delegate to `_format_answer_reply`, reusing the pinned literals byte-identically. Every pre-existing literal is asserted byte-identical by explicit guard tests.

### Why

Riding the classifier contour for the rewriter extends the one landed, hardened light-model pin instead of inventing a second knob set for a same-class task. A pure `retrieve()` extraction lets the enrichment route reuse the exact hybrid-retrieval semantics `/ask` has — same scoping, same RRF, same hit-trace shape — without moving the persistence whose single-decision property D-035 guarantees. The consistency triple makes mislabeled provenance structurally unrepresentable rather than relying on prompt obedience; failing closed (`PARSE_FAILURE`) beats rendering a notes claim nobody can audit. Storing the raw segmented JSON in the trace keeps the one-trace-per-query shape (R-5) without inventing a second trace table for mixed answers. A separate rewrite-trace table keeps RC-2's rows byte-stable and models the real cardinality (zero-or-one rewrite per decision). Degrading a failed rewrite to the original question keeps the route honest and available — the rewrite is an optimization, never a gate.

### Consequence

- **Code:** new `core/chat/rewrite.py` + `core/chat/enriched_prompt.py`; `ChatQueryRewrite` entity; `AnswerResult.model_text` (additive default); new `adapters/chat_routing/rewrite_mock.py` + `rewrite_openai.py` + `build_query_rewriter`; `QueryService.retrieve` + public `normalize_query` + `_persist_trace` delegating to the new `services/retrieval.py` helpers (`RetrievedCandidates`, `build_retrieval_hits`, `SPARSE_MODEL_NAME`, `rrf_leg_score`); `services/routed_chat.py` route execution + narrowed funnel + rewrite-trace persistence; dispatcher `NOTES_PLUS_MODEL` rendering branch + three new literals; webhook composition root + boot-gate rewriter build; additive `MockChatClient` notes-plus-model branch (earlier branches byte-unchanged).
- **Schema:** additive migration `0008.chat-query-rewrites.sql` (new table + UNIQUE decision index + community index); mirrored SQLite inline DDL on fresh files; repository protocol + mock/SQLite/Postgres implementations. `MIGRATION_COUNT` 7 → 8.
- **Tests:** new modules `test_notes_plus_model_prompt.py` (incl. `ESCALATION_CLAUSE` byte pin + consistency triple both directions), `test_chat_query_rewriter_mock.py`, `test_openai_query_rewriter_retry.py` (offline injected fake client; tool shape incl. no-subject-parameter; today-anchored system text; error split), `test_storage_chat_query_rewrites.py` (mock + sqlite + PG-gated; FK + uniqueness + fail-closed scoped reads), `test_routed_chat_notes_plus_model.py` (all grading contours; rewrite-degrade paths; decision-then-rewrite chronology; R-3/R-8 + `subject_scope` forwarded to both legs inside enrichment via a capturing fake search seam; caller-scope-wins; red-flag-style prompt carrying the escalation clause verbatim to a capturing fake provider); extended `test_routed_chat_service.py` (funnel narrowed: `notes_plus_knowledge` is the remaining funnelled route), `test_dispatcher_chat.py` (the four reply shapes byte-pinned; degradation strictly first; empty model segment omitted; failure contours byte-identical; explicit byte-equality guards on every pre-existing sibling literal), `test_chat_client_mock.py`, `test_query_service.py` (`retrieve` purity + kwargs passthrough), `test_domain_models.py` (`model_text` additive default), `test_postgres_migrations.py` (count 8; `_apply_through_0007` stager; 0008 upgrade-preserves-data over a populated 0007 database), `test_end_to_end_smoke.py` (segmented `/chat` round-trip + honest empty-corpus degradation; `/ask` reply unchanged).
- **Validation:** full gate green — ruff + ruff format + mypy strict + pytest (883 passed / 89 skipped offline); PG-gated legs green against the dedicated test database (1013 passed with `MEMORY_RAG_PG_TEST_DSN` set; `test_postgres_mode_imports_cleanly_without_dsn` is DSN-less-only by design and passed in the offline run).
- **Known asymmetry (restated from D-107/D-109):** pre-existing local SQLite files gain neither migration 0008 nor `chat_query_rewrites` (inline DDL on fresh files; no SQLite migration runner); Postgres is the canonical durable backend and migrates via 0008.
- **Untouched:** the `QueryService.answer(message, *, date_range, subject_scope)` seam signature and every `/ask` contour and literal (refactor proven behavior-preserving by the unmodified suites); every pre-existing reply literal; `/sources` stays the `/ask`-only surface (D-109); the unconditional draft floor (I-14 / R-13); raw-before-enrichment (I-3 / R-1); Postgres as sole durable source of truth (I-2). No new I-/R- number; no new Settings knob.

### Out of scope (per packet boundaries)

- Knowledge-source port, Tavily adapter, and `notes_plus_knowledge` (`diary_plus_web`) behavior beyond enum/fallback scaffolding; the outward-query rewrite conditioned on retrieved personal context (RC-4).
- Escalation-invariant verification on the knowledge-source route; consolidated regression suite; RUNBOOK operator section; REAL round-trip evidence; milestone-closure flips incl. `docs/INVARIANTS.md` I-9 reconciliation (RC-4).
- `/sources` integration for `/chat` answers (revisit at RC-4 per D-109).
- The explicit `/ask` filter-syntax item (its natural-language side is now partially delivered by this packet; the explicit-syntax half stays open).
- Natural-language subject scoping (blocked on A-46); A-10 edit/delete (next milestone after RC-4); A-21 / Phase 9 host seams.

---

## D-111 — RC-4: knowledge source + `notes_plus_knowledge` (`diary_plus_web`) + milestone closure

### Context

Packet RC-4 of the routed-chat milestone (D-108; `docs/ROUTED-CHAT-ROADMAP.md`). RC-2 (D-109) landed the classifier/dispatcher seam and the two no-enrichment routes; RC-3 (D-110) landed rewrite-to-kwargs, scoped enrichment retrieval, and the first mixed route, leaving `notes_plus_knowledge` as an enum member funnelled to `notes_lookup` and no knowledge-source port, adapter, or config anywhere. This packet makes the last route real — classify → rewrite-to-kwargs → scoped enrichment retrieval → outward rewrite conditioned on retrieved personal context → knowledge search → one generation citing all three planes (generalized I-9) — and performs the milestone-closure doc reconciliation. Every design decision below implements a clause already fixed by D-108; this entry records the implementation-level choices RC-4 made inside that contract, plus the two owner decisions taken at packet planning.

### Decision

- **Narrow core knowledge-source port (`core/chat/knowledge.py`), not welded to "web".** `KnowledgeSource` exposes `provider_name` and sync `search(query) -> KnowledgeResult(excerpts, raw_output, latency_ms)`; `KnowledgeExcerpt(ref, title, text)` carries `ref` as an **opaque locator** — a URL for the Tavily adapter, any stable reference string for a future curated provider behind the same port (D-108). The failure split mirrors the classifier/rewriter seams (D-035): `KnowledgeSourceUnavailableError` (no output after bounded retries) vs `KnowledgeSourceOutputError` (output existed but unusable, carried verbatim). An empty result list is a valid outcome, not an error.
- **Hardened Tavily adapter over httpx (`adapters/knowledge/tavily.py`) — no new dependency, no SDK.** One HTTPS POST to the Tavily search endpoint via `httpx` (already a runtime dependency); the API key travels **only** in the `Authorization` header — never the request body, the persisted `raw_output`, or logs. R-9 hardening reuses `run_with_retries` with a module-local httpx error classifier (timeouts / transport errors / 5xx / 429 retryable, other 4xx fail fast) and a module-local numeric `Retry-After` extractor; `adapters/resilience.py` itself is untouched (its loop is provider-agnostic via injected callables). `build_knowledge_source` switches on the new `knowledge_backend` knob (`mock` default — the deterministic `MockKnowledgeSource` returns no excerpts unless scripted, so mock-mode answers carry no fabricated web plane).
- **Config + boot gate (R-10).** Three new knobs: `knowledge_backend` (`mock`|`tavily`), `tavily_api_key`, `knowledge_max_results` (default 5 — a tuning placeholder, assumption-flagged as A-47). New `_verify_knowledge_contour`: `tavily` requires a non-empty key; the factory must build; `provider_name` non-empty. `app.created` and `dispatcher.built` log lines gain `knowledge_backend=`. `.env.example` gains `TAVILY_API_KEY` next to `OPENAI_API_KEY` (credentials are the file's existing register; the backend-selection knobs follow the file's existing convention of not being listed — `EMBEDDING_BACKEND` / `CHAT_BACKEND` / `CLASSIFIER_*` are likewise absent, so no classifier-knob reconciliation was needed).
- **Outward rewrite is its own narrow seam riding the classifier contour — no new pin, no new knob.** D-110 deferred the D-108 "retrieve personal context first, rewrite the outward query using it" step to this packet. New `OutwardQueryRewriter` Protocol (`core/chat/outward.py`): `rewrite_outward(question, *, notes_context) -> OutwardRewrite(search_query, ...)`, no `today` parameter (nothing date-relative resolves here). Adapters (`adapters/chat_routing/outward_mock.py` / `outward_openai.py`) ride `classifier_backend` / `classifier_model` exactly like the RC-3 retrieval rewriter (the D-109 "no further knob set" discipline); the OpenAI adapter forces the `rewrite_outward_query` tool at `temperature=0`, renders retrieved chunk texts as context lines, and instructs the model to fold in only the sharpening specifics — never note text wholesale, never names of people. The boot gate builds it through the shared factory so gate and request path cannot disagree.
- **Outward-rewrite failure degrades to the stripped original question, not the retrieval query.** The RC-3 retrieval rewrite is keyword-optimized for notes search — the wrong register for an external search; the original question is the honest, cause-neutral floor. Failure (no rewriter wired / provider unavailable / unusable output / rewrite stripping to empty) never raises; logs name the cause (`chat.outward_rewrite_failed reason=…`), user-facing wording does not.
- **Knowledge-search failure degrades within the route, not by rerouting.** Mirroring RC-3's rewrite-degrade precedent (the search is an enrichment plane, never a gate): after bounded retries the route proceeds with an **empty knowledge plane** — generation still runs (the route's point), the parser structurally forces the empty plane (with no offered refs any citation is fabricated → fail closed), the reply renders no web segment, and the trace row records the failure truthfully (`raw_output` verbatim on the unusable-output contour, `""` on the no-output contour; `result_count=0`). Logged as `chat.knowledge_failed reason=…`; cause-neutral to the user. With **no knowledge source wired at all**, the route stays non-dispatchable and funnels to `notes_lookup` exactly like RC-2 (requested-vs-effective preserved, R-6).
- **`notes-plus-knowledge-v1` prompt contract (`core/chat/knowledge_prompt.py`).** Mirrors `notes-plus-model-v1` plus the knowledge plane: six response fields `{notes_text, cited_chunk_ids, knowledge_text, cited_knowledge_refs, model_text, notes_uncertainty}`. `model_text` stays — synthesis prose that is neither note- nor knowledge-grounded needs a labeled home or it would leak into the web plane unlabeled (I-9). The **escalation clause is imported from `enriched_prompt.py`, not copied** — one byte-pinned constant shared by both mixed routes (D-110's "RC-4 reuses it verbatim" made structural). Parsing keeps the RC-3 notes consistency triple verbatim and adds: fabricated `cited_knowledge_refs` rejected against the offered excerpt refs, and the knowledge consistency pair `knowledge_text == "" ⇔ cited_knowledge_refs == ()` enforced both directions. There is deliberately **no knowledge-uncertainty marker**: the D-108 honest-degradation clause keys on diary evidence only; `notes_uncertainty` drives `FallbackMode` exactly as RC-3 and the knowledge plane is present-or-absent. `AnswerPrompt` gains additive `knowledge_refs` (the offered refs travel to the parser and the mock provider without scraping `user_text`); `AnswerResult` gains additive `knowledge_text` / `knowledge_refs`.
- **One additive trace table folding outward-rewrite provenance in (`0009.chat-knowledge-searches.sql`).** `chat_knowledge_searches` records, per `notes_plus_knowledge` execution: the always-present `outward_query` (the rewrite or the degraded-to original question — what was actually searched), the outward-rewriter provenance triple (`model_name` `''` only when none wired; `raw_output` per the D-035 truthful-provenance rule), and the provider result provenance (`provider_name`, `result_count`, verbatim `raw_output`, `latency_ms`). One row per execution **including failed searches**; zero rows for funnelled or retrieval-seam-unavailable calls (the RC-3 no-rows shape). The outward rewrite and the search are one pipeline step's trace with the same zero-or-one-per-decision cardinality — folding them avoids a second table, FK + UNIQUE per decision mirror 0008, and the row is written strictly after the decision row. The 0008 migration comment's "one row per notes_plus_model execution" is **historical** (migrations are immutable); both mixed routes now write 0008 rows and the `ChatQueryRewrite` docstring carries the current statement.
- **Reply rendering: one new pinned literal, "web" register deliberate.** `_format_notes_plus_knowledge_reply` renders (only on an exact requested match — never the reroute trailer): provider/parse failures delegating to the shared formatter (pinned literals byte-identical) → the RC-3 degradation headers byte-reused, strictly before any web/model content → the notes segment under `_LABEL_CHAT_NOTES` → the knowledge segment under the **new** `_LABEL_CHAT_WEB = "From the web:"` with `knowledge_text` followed by each cited ref (URL) verbatim on its own line → the model segment under `_TRAILER_MODEL_KNOWLEDGE`. Empty segments are omitted entirely (no orphan labels). The user-facing "web" label vs the core "knowledge" register is deliberate: the only live provider is a web search; the literal can be revisited when a non-web provider lands behind the same port. Every pre-existing literal is asserted byte-identical by a new guard test.
- **Owner decision — `/sources` stays the `/ask`-only surface.** The D-109 revisit is resolved for this milestone: no `/chat` integration, no code change; the existing both-direction pins stay authoritative. Revisitable after the milestone as a product decision.
- **Owner decision — evidence leg is prep + operator drill (REAL-1.0/1.1 precedent, D-073/D-074).** RC-4 commits the routed-chat drill template (`docs/routed-chat-drill/routed-chat-smoke-TEMPLATE.json`) and the RUNBOOK procedure; the operator runs the round trip out-of-band with real OpenAI + Tavily keys and commits the dated, redaction-checked `docs/routed-chat-drill/routed-chat-smoke-<YYYYMMDD>-evidence.json`. **Milestone closure is conditional on that artifact** — the roadmap / execution-map / todo flips say "code-complete; closes on the operator's dated evidence artifact", not "closed".
- **Privacy seam surfaced, not hidden.** The outward query intentionally carries distilled personal context to the external search provider — recorded here and in the RUNBOOK operator section (with the system-prompt mitigations: no wholesale note text, no names of people), per the fallback/provenance discipline.

### Why

A locator-shaped `ref` keeps the port honest to D-108's "not welded to web" while letting the reply layer satisfy "web (cited URLs)" verbatim today. Folding outward-rewrite provenance into the knowledge-search row models the real cardinality and halves the trace surface a second table would cost. Degrading within the route (empty plane) rather than rerouting keeps the classifier's verdict meaningful and the requested-vs-effective signal clean — a mid-route provider failure is not a classification event. Importing the escalation clause makes byte-drift between the two mixed routes structurally impossible rather than test-detected. Ref-subset validation extends the I-9 fabricated-citation defense to the web plane: provenance nobody can audit fails closed as `PARSE_FAILURE` instead of rendering. The stripped original question as the outward floor is what external search engines handle best and carries no notes-register artifacts. Conditional-closure wording keeps the docs truthful: the §6 exit criterion demands real-backend evidence this packet cannot honestly self-certify.

### Consequence

- **Code:** new `core/chat/knowledge.py` + `core/chat/outward.py` + `core/chat/knowledge_prompt.py`; `ChatKnowledgeSearch` entity; additive `AnswerPrompt.knowledge_refs` + `AnswerResult.knowledge_text`/`knowledge_refs`; new package `adapters/knowledge/` (factory + mock + Tavily over httpx); new `adapters/chat_routing/outward_mock.py` + `outward_openai.py` + `build_outward_rewriter`; `services/routed_chat.py` route execution (`_rewrite_outward`, `_answer_notes_plus_knowledge`, `_KnowledgeCapture`, narrowed funnel gated on a wired source, knowledge-row persistence, `chat.knowledge` / `chat.outward_rewrite_failed` / `chat.knowledge_failed` log lines); dispatcher `NOTES_PLUS_KNOWLEDGE` rendering branch + one new literal; `config.py` three knobs; `app.py` `_verify_knowledge_contour` + outward-rewriter build in the classifier gate; webhook composition root; additive `MockChatClient` notes-plus-knowledge branch (earlier branches byte-unchanged); `.env.example` `TAVILY_API_KEY`.
- **Schema:** additive migration `0009.chat-knowledge-searches.sql` (new table + UNIQUE decision index + community index); mirrored SQLite inline DDL on fresh files; repository protocol + mock/SQLite/Postgres implementations. `MIGRATION_COUNT` 8 → 9.
- **Tests:** new modules `test_knowledge_source_mock.py`, `test_knowledge_source_tavily.py` (httpx mock transport: result mapping, key-only-in-header, `max_results` cap, bounded retries incl. clamped `Retry-After`, 4xx fail-fast, output-error verbatim body), `test_outward_rewriter_mock.py`, `test_openai_outward_rewriter_retry.py` (offline injected fake client; tool shape; notes-context conditioning; error split), `test_notes_plus_knowledge_prompt.py` (six-field shape; fabricated chunk-id and ref rejection; notes triple + knowledge pair both directions; escalation clause identity **and** byte pin), `test_storage_chat_knowledge_searches.py` (mock + sqlite + PG-gated; FK + uniqueness + fail-closed scoped reads; failed-search shape), `test_routed_chat_notes_plus_knowledge.py` (dispatch vs source-`None` funnel; all grading contours; outward-degrade to the stripped question; in-route search-failure degradation; decision→rewrite→knowledge chronology; R-3/R-8 + `subject_scope` on both legs; no-rows `NotImplementedError` contour; escalation clause verbatim + specialist-recommendation answer passing through unaltered), `test_boot_knowledge_gate.py`, and the **consolidated milestone suite `test_routed_chat_milestone.py`** (all four routes end-to-end at service granularity, mock + PG-gated parametrization, fallback default on the empty question, cross-community isolation through routed answers, one-decision-per-call uniformity; docstring indexes the per-packet suites it composes with); extended `test_dispatcher_chat.py` (three-segment shape pinned; degradation-first; empty-plane omission; failure delegation; new RC-4 literal pinned with every pre-existing sibling literal byte-identical), `test_chat_client_mock.py`, `test_domain_models.py`, `test_postgres_migrations.py` (count 9; `_apply_through_0008` stager; 0009 upgrade-preserves-data; stagers exclude 0009 so prior-version staging stays genuine), `test_end_to_end_smoke.py` (three-segment `/chat` round-trip with refs rendered; `/ask` unchanged).
- **Validation:** full gate green — ruff + ruff format + mypy strict + pytest (994 passed / 96 skipped offline); PG-gated legs green against the dedicated test database (1138 passed with `MEMORY_RAG_PG_TEST_DSN` set; `test_postgres_mode_imports_cleanly_without_dsn` is DSN-less-only by design — verified failing identically on unmodified RC-3 HEAD with a DSN exported — and passed in the offline run).
- **Docs:** this entry; `docs/ROUTED-CHAT-ROADMAP.md` (Status, §2 web row, §4 RC-4 row, §6 wording) / `docs/execution-map.md` (RC-4 row) / `docs/todo.md` (RC block; A-10 stays visibly queued-next) flipped to **code-complete, closure conditional on the operator's dated evidence artifact**; `docs/INVARIANTS.md` I-9 clause reconciled to enforced-in-code wording; `docs/product/TechSpec.md` §4 `/chat` line reconciled (all four routes dispatchable); `docs/GLOSSARY.md` stale "until RC-2" note flipped to landed; new `docs/RUNBOOK.md` "Routed chat" operator section + drill subsection; new `docs/routed-chat-drill/routed-chat-smoke-TEMPLATE.json`; `docs/assumptions.md` + `docs/assumption-audit.md` open **A-47** (knowledge-search defaults: `knowledge_max_results=5`, Tavily request shape minimal — no `search_depth`/topic/domain filters — and ref=URL under the Tavily adapter).
- **Known asymmetry (restated from D-107/D-109/D-110):** pre-existing local SQLite files gain neither migration 0009 nor `chat_knowledge_searches` (inline DDL on fresh files; no SQLite migration runner); Postgres is the canonical durable backend and migrates via 0009.
- **Untouched:** every `/ask` contour and literal; every pre-existing `/chat` reply literal (guard-tested); `/sources` stays the `/ask`-only surface (owner decision above); the unconditional draft floor (I-14 / R-13); raw-before-enrichment (I-3 / R-1); Postgres as sole durable source of truth (I-2); `adapters/resilience.py`; the `QueryService` seams. No new I-/R- number; no change to the D-037 / RC-2 model pins.

### Out of scope (per packet boundaries)

- The operator execution of the routed-chat drill and the dated evidence artifact (the milestone's final closure signal; out-of-band, requires real OpenAI + Tavily keys).
- A curated domain-knowledge provider behind the same port; a synthetic-ref scheme for web citations (refinable follow-up only if drill evidence shows URL-ref churn under `response_format=json_object`).
