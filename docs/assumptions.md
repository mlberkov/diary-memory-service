...
- A-45 → D-097 (subject-scoping bootstrap/assignment contract: an opaque / community-scoped / **nullable** `subject_id` on `Note` / `EventChunk`, born directly as `subject_id` — `child` stays a use-case label; assignment is an adapter-axis function with a **default single-subject mapping**; `null` = community-wide; **no** core subject registry/entity; an **optional** retrieval filter mirroring the D-040 `date_range` seam; **separate from A-15 visibility**. Resolved at the contract level only; the code realization is sequenced as Milestone H / H-1..H-4 in `docs/SUBJECT-SCOPING-ROADMAP.md`).
