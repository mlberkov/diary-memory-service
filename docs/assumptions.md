...
- **A-45. Subject/child bootstrap**: how a `subject` (first use case: `child`) is first created and assigned to notes within a community. **Carved out of A-14 by D-093** (which resolved the community-bootstrap / mapping / membership half). `subject_id` is born in the D-040 child-filter lineage (`docs/GLOSSARY.md`, TechSpec §5); this assumption is owned by that lineage and resolves when it lands. Required before subject-scoped filtering / retrieval.
- **A-15. Visibility scopes**: enumerated values for `visibility_scope` are undefined. **Sequenced by D-093 after the first grouped-diary pilot** — community-level scoping (every chat member reads the community corpus) is the access model until then, and Slice 8.2 stays blocked on this. Required before Phase 8 / Slice 8.2.
