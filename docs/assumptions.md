...
- **A-38. Draft lifecycle semantics**: under the draft-by-default floor (D-027), it is not yet specified how long an unpromoted draft is retained, whether it expires by inactivity or by explicit cleanup, and which exact user action promotes a draft to a note (a follow-up `/note` referencing it, a UI confirmation, an inline command, etc.). The lifecycle field on `SourceMessage` that records draft-vs-note state is also unspecified (the existing `detected_route` may be extended, replaced, or kept alongside a new column). Required before the draft routing implementation packet.
- **A-39. Raw export packaging and delivery**: D-027 commits the formats (JSON and TXT) and the scope (raw `SourceMessage` rows within the requester's scope). Open: per-host delivery channel (Telegram file reply, HTTP download endpoint, host-app screen), the request shape (time-range arguments, async generation vs inline), inclusion of derived state as an optional flag, and the audit-row schema for export provenance. Required before the export implementation packet.
