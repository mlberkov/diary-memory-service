...
| A-44 | Author display-name resolution (adapter-only resolution; fallback `username → first_name → opaque short-ID`; host-supplied / non-authoritative; `/sources` is the sole milestone surface; answer-reply attribution deferred) | Telegram display name mistaken for identity; inconsistent / missing attribution across surfaces | **Resolved by D-086**: capture/persistence shape pinned by D-082; landing seam pinned by D-083 (Option A — adapter-owned side table + port); capture + durable landing implemented by D-084; resolution helper + `/sources` rendering implemented by D-086 (adapter-only, names non-authoritative) | human + agent | **closed** — author display-name capture / rendering packet (opened → D-081; capture/persistence shape → D-082; landing seam → D-083; capture + durable landing → D-084; resolution + `/sources` rendering → D-086). Answer-reply / `/ask`-reply attribution remains a separate deferred placeholder, not part of this closure; its **contract** is pinned by D-091 (contributor attribution over the grounding chunks, adapter-resolved via the same D-086 seam), rendering deferred to D-091's follow-up code packet — A-44 stays closed, not reopened |
