...
5. **Tenant / auth mapping** — maps the host's identity model (Telegram chat → family scope; TheyGrow account → workspace; OSS deployment → single-tenant default) onto the core's scope. The mapping function is adapter; the scoped query is core. Resolving an author's **display name** from host-supplied identity fields is likewise adapter-side; the core carries only the opaque `author_user_id` (D-081, A-44). Capturing and persisting those host identity fields as an adapter/storage-owned snapshot is likewise adapter-side (D-082); that snapshot lands in a separate adapter-owned side table written through an adapter-owned storage port distinct from the core `DomainRepository` (D-083). The Telegram chat → community mapping is ratified by D-093 (implicit-on-first-message bootstrap; default 1:1 from `external_chat_id`; the core receives an opaque `community_id`; membership inherited from host-chat membership); many communities may coexist on one instance. That mapping is realized by a single adapter-owned resolver `resolve_community_id` (`adapters/telegram/community.py`, D-094): the adapter resolves the opaque `community_id` at the edge and carries it on `InboundMessage.community_id`, so the core scopes on the opaque id and never re-derives scope from the chat id (I-1). Grouped + multi-diary-on-one-instance is supported and documented operator-facing in `docs/RUNBOOK.md` (D-096). Host→subject assignment is the same axis one level finer (D-097): the single adapter-owned resolver `resolve_subject_id` (`adapters/telegram/subject.py`, H-2) resolves an opaque, community-subordinate, nullable `subject_id` at the edge (default single-subject mapping → `None` = community-wide) and carries it on `InboundMessage.subject_id`; the core persists and optionally filters on it (D-107) but never derives subject from a host identity field.
