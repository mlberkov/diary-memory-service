...
## Schema evolution
- **A-34. Non-destructive schema-changing upgrades are not yet demonstrated**: OP-1.1 (D-045) landed the schema-migration foundation — `yoyo-migrations`, a baseline migration capturing the current schema verbatim, a bootstrap that applies migrations to head, and a documented one-time `stamp` step for adopting pre-existing local Postgres volumes without a destructive reset. The destructive `docker compose down -v` contour is no longer the upgrade path. **A-34 stays open**: OP-1.1 introduces no schema-*changing* migration beyond the baseline, so the OP-1 exit criterion — a non-destructive upgrade from a prior schema version succeeding without a volume reset — is not yet exercised. SQLite is not migration-managed and still picks up its schema on a fresh DB file. A-34 closes, and the OP-1 group completes, when a later packet introduces and validates a real schema-changing upgrade migration. Stage-2 packet group **OP-1** (D-044; see `docs/OPERATIONALIZATION-ROADMAP.md`).

## Phase 3.1+3.2 indexing contour (current)
- **A-35. Sync indexing, no auto-retry**: `DomainService.ingest` calls the `EmbeddingClient` synchronously after `save_event_chunks` commits (D-024). On provider failure the chunks remain persisted, `embedding_status` flips to `failed`, and zero `embedding_records` are written for that source; the ingest result still returns `FallbackMode.NONE` (raw + chunks survived — I-2, I-3). Failed embeddings stay failed; replay (R-2) does not retry. Reconciliation for failed rows is sequenced as Stage-2 packet group **OP-3** (D-044; see `docs/OPERATIONALIZATION-ROADMAP.md`), which introduces bounded retries and a dead-letter strategy after OP-2 provider hardening — A-35 stays open until OP-3 lands. The SQL probe `SELECT chunk_id FROM event_chunks WHERE embedding_status='failed'` is the operator inspection surface until then.
