...
| A-43 | Observability scope for the first DEPLOY-1 VPS contour (logs-first with a forward seam to remote sinks; specific surface and tooling not pinned) | Operator-blind on the first VPS contour; observability bolted on late | Pinned by the DEPLOY-1.x packet that ships observability (see `SELF-HOSTED-DEPLOYMENT-ROADMAP.md` §4) | agent | DEPLOY-1.x observability packet |
