...
| A-43 | Observability scope for the first DEPLOY-1 VPS contour (logs-first with a forward seam to remote sinks; specific surface and tooling not pinned) | Operator-blind on the first VPS contour; observability bolted on late | Pinned by the DEPLOY-1.x packet that ships observability (DEPLOY-1.6 / D-065 did NOT fold it in — the off-box sink reused the existing `pg_backup.*` log-prefix family without forcing a new logging contract; see `SELF-HOSTED-DEPLOYMENT-ROADMAP.md` §4) | agent | later DEPLOY-1.x packet (not DEPLOY-1.6) |
