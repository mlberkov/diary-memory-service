...
## Phase 4 — Slice 4.4 evidence rendering (next, after Slice 4.3b)
- Owner: agent
- Map: execution-map 4.4
- Concrete: with `ChatClient`, `AnswerTrace` persistence, and answer-side fallback grading landed in Slices 4.3a + 4.3b (D-034 + D-035), switch the Telegram reply layer from the evidence-bullets shape to `AnswerResult.answer_text` plus citation lines. The Dispatcher already emits per-`FallbackMode` trailers; this packet adds the citation rendering so the user sees the LLM-produced answer alongside the chunks it cited, while preserving the requested-vs-effective signaling Slice 4.3b introduced. Stay bounded — do not widen into Phase 6 real-provider adapters or into search-quality work.
