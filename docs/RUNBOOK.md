...
Answer-reply contributor attribution (D-091 contract, surfaced by D-092): the `/ask` grounded reply carries a `Contributors: …` footer line beneath `answer_text` naming the **distinct contributors who grounded the answer** — the distinct authors (by opaque `author_user_id`) of the answer's grounding chunks (the same chunks `/sources` recalls), deduped by `author_user_id` and listed in first-appearance order, rendered only when the grounding set is non-empty (so grounded replies — including `WEAK_EVIDENCE` / `AMBIGUOUS`, which still carry grounding chunks — get the footer; `NO_EVIDENCE` / empty-query / `PROVIDER_UNAVAILABLE` replies carry none and are unchanged). It reuses the same adapter-only resolver (`resolve_chunk_author_display`, requester-`community_id`-scoped per D-089) and the D-086 fallback chain, so the same non-authoritative / opaque-floor semantics apply. The channel-neutral dispatcher carries the opaque grounding chunks on the ASK `DispatchResult` (`grounding_chunks`, mirroring `source_chunks`) and the Telegram adapter (`adapters/telegram/webhook.py`) resolves the distinct contributors and appends the footer — display resolution never happens in the core (I-1 / I-6 / I-7 preserved). Surfaced by D-092 (the code packet for D-091's contract).
