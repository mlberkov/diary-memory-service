...
Generalized for routed answers in D-108 (routed-chat milestone): every answer segment carries an explicit provenance class — **notes** (cited through the existing cited-only evidence surface; this clause changes nothing for the notes-grounded class, whose contract above stands verbatim), **web** (cited URLs), or **model** (explicitly labeled as model knowledge). No segment may present model- or web-sourced content as if it came from the diary. Enforced in code by the routed-chat packets RC-2..RC-4 (D-109 / D-110 / D-111; `docs/ROUTED-CHAT-ROADMAP.md`): the segmented-answer parsers reject fabricated chunk citations against the prompt context and fabricated knowledge refs against the offered excerpts (failing closed as `PARSE_FAILURE` rather than rendering), consistency rules make an uncited notes or knowledge claim structurally unrepresentable, and the reply layer labels the web and model segments explicitly — the web segment's cited refs render verbatim.
