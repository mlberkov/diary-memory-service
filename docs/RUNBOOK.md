...
`/drafts` blocks render the author as a resolved display name — the same adapter-only D-086 ladder `/sources` uses (`@username → first_name → opaque `user-<last8>` floor`), requester-`community_id`-scoped (D-089) — instead of the raw opaque `author_user_id`. The block header is `📝 <created_at ISO> · <author>` then a blank line then the verbatim draft text; the earlier `author:<id> · id:<short>` technical segments are dropped. Resolution stays adapter-side via the internal helper `adapters/telegram/author_display._resolve_source_author_display`: a draft is already a community-scoped `SourceMessage`, so the snapshot is read directly by its `(external_chat_id, external_message_id, edit_seq)` tuple — **no** `get_source_message` bridge — and a missing / withheld snapshot falls through to the opaque floor (never blank, never a raise). Authorship is still carried only as the opaque `author_user_id` (I-6); the display name is non-authoritative presentation. This makes `/drafts` consistent with the cited-only `/sources` surface — the D-098 "evidence-faithful answer & source attribution" milestone.
