...
Answer-side trace persistence is enforced on the success and no-evidence/empty-query contours as of Slice 4.3a (D-034): every `/ask` reply also writes one `AnswerTrace` row (FK to `queries.query_id`, UNIQUE on `query_id`) recording `prompt_version`, `context_chunk_ids`, `answer_text`, `model_name`, `token_counts`, `latency_ms`, and `fallback_mode`. Weak-evidence / ambiguous / provider-unavailable grading and the corresponding marker semantics remain deferred to Slice 4.3.
