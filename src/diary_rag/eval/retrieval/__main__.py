...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="python -m diary_rag.eval.retrieval")
...
def _run_postgres(
    *,
    ...
    dsn = os.environ.get("DIARY_RAG_PG_TEST_DSN")
    if not dsn:
        raise RuntimeError(
        ...
        def lookup(query: str) -> list[float]:
            if query not in cache:
                raise KeyError(
                    ...
                    f"regenerate via "
                    f"`uv run python -m diary_rag.eval.retrieval.regenerate_embeddings`"
                )
            return cache[query]
