...
(exponential delay, jitter, clamped `Retry-After`). OP-3 is in progress —
OP-3.1 (D-050) landed the failed-embedding discovery seam and the read-only
reconciliation entrypoint, and OP-3.2a (D-051) landed the retry execution that
transitions succeeded chunks `failed → ready`; the OP-3.2b slice that routes
exhausted retries to the dead-letter surface and resolves A-35 is not yet
executed. No other `OP-` packet has been executed. This document records the
...
| **OP-3** *(in progress — OP-3.1 / D-050 and OP-3.2a / D-051 landed; OP-3.2b open)* | Failed-embedding reconciliation | OP-1 and OP-2 merged — OP-3 consumes OP-2's bounded-backoff retry (6.1) and dead-letter (6.2) primitives, per A-35's own specification. | A-35 resolved (`docs/assumptions.md`), governed by the same Phase 6 Definition of Done. OP-3.1 landed the read-only discovery seam and OP-3.2a the retry execution (`failed → ready`); neither resolves A-35 — OP-3.2b (exhausted-retry dead-letter routing) does. | `make check`; a `failed` chunk is retried, succeeds or lands in the dead-letter surface, and the outcome is observable. |
