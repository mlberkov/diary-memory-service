...
## Phase 4 — Chat client seam + AnswerTrace (next, after Slice 4.2)
- Owner: agent
- Map: execution-map 4.3 → 4.4 (the chat-client seam attaches to the Slice 4.2 contract)
- Concrete: with the versioned prompt + structured-answer schema landed in Slice 4.2 (D-033), build the LLM-call seam on top. `ChatClient` Protocol with a mock implementation (honest provenance, `model_name="mock"`); call `build_answer_prompt(context)` and `parse_structured_answer(raw, context=context)` around the chat-client invocation; persist `AnswerTrace(query_id, prompt_version, context_chunk_ids, answer_text, confidence_band, fallback_mode, model_name, token_counts, latency_ms, created_at)` — its own table, FK to `queries.query_id`. Bound the packet — pick one slice (e.g. `ChatClient` seam + `AnswerTrace` persistence on the success / no-evidence contours) and defer weak-evidence / ambiguous / provider-unavailable grading (Slice 4.3) and Telegram citation rendering (Slice 4.4) to follow-on packets.
