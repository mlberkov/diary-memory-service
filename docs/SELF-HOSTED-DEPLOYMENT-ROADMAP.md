...
worktree under `mktemp -d`. Preflight, not closure — at that point
DEPLOY-1.7 was not started and DEPLOY-1 remained open. DEPLOY-1.7a
landed (D-067) — clean-VPS pilot smoke + off-box backup §2-invariant
verification against a real S3-compatible target via a new
"Clean-VPS pilot smoke (DEPLOY-1.7a / D-067)" subsection in
`docs/RUNBOOK.md` and a committed pilot-smoke evidence artifact at
`docs/deploy1-drill/deploy1-pilot-smoke-20260527-evidence.json`.
DEPLOY-1.7 was split: 1.7a closes the pilot-smoke + off-box halves
now; DEPLOY-1.7b (v2 → v3 cross-version upgrade drill on a real
previously-installed v2 VPS) is the sole canonical remaining closure
packet for DEPLOY-1. DEPLOY-1 remains open.**
...
the sole canonical remaining packet for DEPLOY-1 closure.
