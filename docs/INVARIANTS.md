...
Absence of an explicit command never causes silent discard, downgrade, or upgrade of raw persistence. Drafts are the safety floor (D-027 / D-028) and, per D-078, the only route for command-less plain text: heuristics do not auto-route plain text to NOTE or ASK — those lifecycles are reached only via the explicit `/note` / `/ask` commands. CLARIFY (D-020) remains valid only as a response when an explicit command actively conflicts with intent, not as a plain-text route and not as the persistence floor. (D-078 records this contract; the classifier code change that enforces it lands in a later packet of the Stage-1 capture/routing baseline correction.)
